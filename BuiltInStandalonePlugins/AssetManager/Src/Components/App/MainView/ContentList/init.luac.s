PROTO_0:
  GETUPVAL R0 0
  GETUPVAL R2 1
  NAMECALL R0 R0 K0 ["handleRightClick"]
  CALL R0 2 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R7 0
  GETTABLE R6 R7 R1
  JUMPIFEQ R6 R0 [+2]
  LOADB R5 0 +1
  LOADB R5 1
  FASTCALL2K ASSERT R5 K0 [+4]
  LOADK R6 K0 ["Row index does not match row"]
  GETIMPORT R4 K2 [assert]
  CALL R4 2 0
  DUPTABLE R4 K7 [{"Key", "Position", "AssetInfo", "Size"}]
  SETTABLEKS R1 R4 K3 ["Key"]
  SETTABLEKS R2 R4 K4 ["Position"]
  SETTABLEKS R0 R4 K5 ["AssetInfo"]
  SETTABLEKS R3 R4 K6 ["Size"]
  RETURN R4 1

PROTO_2:
  GETUPVAL R2 0
  MOVE R4 R1
  NAMECALL R2 R2 K0 ["requestNextPage"]
  CALL R2 2 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["new"]
  CALL R1 0 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["use"]
  CALL R2 0 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K1 ["use"]
  CALL R3 0 1
  GETUPVAL R4 3
  MOVE R5 R3
  CALL R4 1 1
  GETUPVAL R6 4
  GETTABLEKS R5 R6 K2 ["createElement"]
  GETUPVAL R6 5
  NEWTABLE R7 4 0
  GETTABLEKS R8 R0 K3 ["LayoutOrder"]
  SETTABLEKS R8 R7 K3 ["LayoutOrder"]
  NEWCLOSURE R8 P0
  CAPTURE VAL R3
  CAPTURE VAL R2
  SETTABLEKS R8 R7 K4 ["OnRightClick"]
  GETUPVAL R9 4
  GETTABLEKS R8 R9 K5 ["Tag"]
  LOADK R10 K6 ["ContentList X-Top X-Column X-Center %*"]
  GETUPVAL R14 4
  GETTABLEKS R13 R14 K5 ["Tag"]
  GETTABLE R12 R0 R13
  NAMECALL R10 R10 K7 ["format"]
  CALL R10 2 1
  MOVE R9 R10
  SETTABLE R9 R7 R8
  DUPTABLE R8 K10 [{"HeaderRow", "ListContainer"}]
  GETUPVAL R10 4
  GETTABLEKS R9 R10 K2 ["createElement"]
  GETUPVAL R10 6
  DUPTABLE R11 K11 [{"LayoutOrder"}]
  NAMECALL R12 R1 K12 ["getNextOrder"]
  CALL R12 1 1
  SETTABLEKS R12 R11 K3 ["LayoutOrder"]
  CALL R9 2 1
  SETTABLEKS R9 R8 K8 ["HeaderRow"]
  GETUPVAL R10 4
  GETTABLEKS R9 R10 K2 ["createElement"]
  GETUPVAL R10 5
  NEWTABLE R11 2 0
  NAMECALL R12 R1 K12 ["getNextOrder"]
  CALL R12 1 1
  SETTABLEKS R12 R11 K3 ["LayoutOrder"]
  GETUPVAL R13 4
  GETTABLEKS R12 R13 K5 ["Tag"]
  LOADK R13 K13 ["ItemList"]
  SETTABLE R13 R11 R12
  DUPTABLE R12 K15 [{"List"}]
  GETUPVAL R14 4
  GETTABLEKS R13 R14 K2 ["createElement"]
  GETUPVAL R14 7
  DUPTABLE R15 K22 [{"LayoutOrder", "GetRowProps", "Rows", "RowComponent", "RowHeight", "OnLoadRange", "ScrollingDirection"}]
  NAMECALL R16 R1 K12 ["getNextOrder"]
  CALL R16 1 1
  SETTABLEKS R16 R15 K3 ["LayoutOrder"]
  NEWCLOSURE R16 P1
  CAPTURE VAL R4
  SETTABLEKS R16 R15 K16 ["GetRowProps"]
  SETTABLEKS R4 R15 K17 ["Rows"]
  GETUPVAL R16 8
  SETTABLEKS R16 R15 K18 ["RowComponent"]
  GETUPVAL R16 9
  LOADK R18 K19 ["RowHeight"]
  NAMECALL R16 R16 K23 ["GetAttribute"]
  CALL R16 2 1
  SETTABLEKS R16 R15 K19 ["RowHeight"]
  NEWCLOSURE R16 P2
  CAPTURE VAL R3
  SETTABLEKS R16 R15 K20 ["OnLoadRange"]
  GETIMPORT R16 K26 [Enum.ScrollingDirection.XY]
  SETTABLEKS R16 R15 K21 ["ScrollingDirection"]
  CALL R13 2 1
  SETTABLEKS R13 R12 K14 ["List"]
  CALL R9 3 1
  SETTABLEKS R9 R8 K9 ["ListContainer"]
  CALL R5 3 -1
  RETURN R5 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AssetManager"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["React"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["Framework"]
  CALL R2 1 1
  GETTABLEKS R3 R2 K9 ["ContextServices"]
  GETTABLEKS R4 R3 K10 ["Localization"]
  GETTABLEKS R5 R2 K11 ["UI"]
  GETTABLEKS R6 R5 K12 ["Pane"]
  GETTABLEKS R7 R5 K13 ["List"]
  GETIMPORT R8 K5 [require]
  GETIMPORT R10 K1 [script]
  GETTABLEKS R9 R10 K14 ["HeaderRow"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETIMPORT R11 K1 [script]
  GETTABLEKS R10 R11 K15 ["ItemRow"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R13 R0 K16 ["Src"]
  GETTABLEKS R12 R13 K17 ["Controllers"]
  GETTABLEKS R11 R12 K18 ["ItemsController"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K16 ["Src"]
  GETTABLEKS R13 R14 K19 ["Hooks"]
  GETTABLEKS R12 R13 K20 ["useItems"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R14 R0 K16 ["Src"]
  GETTABLEKS R13 R14 K21 ["Types"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETIMPORT R15 K1 [script]
  GETTABLEKS R14 R15 K22 ["styles"]
  CALL R13 1 1
  GETTABLEKS R15 R2 K23 ["Util"]
  GETTABLEKS R14 R15 K24 ["LayoutOrderIterator"]
  DUPCLOSURE R15 K25 [PROTO_3]
  CAPTURE VAL R14
  CAPTURE VAL R4
  CAPTURE VAL R10
  CAPTURE VAL R11
  CAPTURE VAL R1
  CAPTURE VAL R6
  CAPTURE VAL R8
  CAPTURE VAL R7
  CAPTURE VAL R9
  CAPTURE VAL R13
  RETURN R15 1
