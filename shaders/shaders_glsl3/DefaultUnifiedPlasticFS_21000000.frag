...
void main()
{
    ...
    float f36 = f26.z;
    float f37 = f23 * 5.0;
    vec3 f38 = mat3(vec3(CB0[58].xyz), vec3(CB0[59].xyz), vec3(CB0[60].xyz)) * f25;
    bvec3 f39 = bvec3(!(CB0[58].x == 2.0));
    vec3 f40 = vec4(f25, f37).xyz;
    vec3 f41 = textureLod(PrefilteredEnvIndoorTexture, f40, f37).xyz;
    vec3 f42;
    if (CB0[32].w == 0.0)
    {
        f42 = f41;
    ...
    {
        f42 = mix(f41, textureLod(PrefilteredEnvBlendTargetTexture, f40, f37).xyz, vec3(CB0[32].w));
    }
    ...
    float f56 = f49.z;
    vec3 f57 = -CB0[16].xyz;
    float f58 = dot(f14, f57) * ((1.0 - ((step(f35.x, f36) * clamp(CB0[29].z + (CB0[29].w * abs(f36 - 0.5)), 0.0, 1.0)) * f35.y)) * f33.y);
    ...
    vec3 f70 = (((((((vec3(1.0) - (f46 * f22)) * (((((((CB0[40].xyz * f51) + (CB0[42].xyz * f52)) + (CB0[44].xyz * f53)) + (CB0[41].xyz * f54)) + (CB0[43].xyz * f55)) + (CB0[45].xyz * f56)) + (((((((CB0[34].xyz * f51) + (CB0[36].xyz * f52)) + (CB0[38].xyz * f53)) + (CB0[35].xyz * f54)) + (CB0[37].xyz * f55)) + (CB0[39].xyz * f56)) * f34))) * 1.0) + ((CB0[32].xyz + (CB0[33].xyz * f34)) * 1.0)) + ((((vec3(1.0) - (f67 * f22)) * CB0[15].xyz) * f60) + (CB0[17].xyz * clamp(-f58, 0.0, 1.0)))) + (f32.xyz * (f32.w * 120.0))) * f19.xyz) + (((mix(f42, textureLod(PrefilteredEnvTexture, vec4(vec3(f39.x ? f38.x : f25.x, f39.y ? f38.y : f25.y, f39.z ? f38.z : f25.z), f37).xyz, f37).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f25.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f34)) * f46) * f22) + (((f67 * (((f68 + (f68 * f68)) / (((f69 * f69) * ((f63 * 3.0) + 0.5)) * ((f62 * 0.75) + 0.25))) * f60)) * CB0[15].xyz) * 1.0));
    vec4 f71 = vec4(0.0);
    f71.x = f70.x;
    vec4 f72 = f71;
    f72.y = f70.y;
    ...
    vec3 f76 = textureLod(PrefilteredEnvTexture, vec4(f24, 0.0).xyz, max(CB0[18].y, f75) * 5.0).xyz;
