...
void main()
{
    ...
    float f42 = f32.z;
    float f43 = f27 * 5.0;
    vec3 f44 = mat3(vec3(CB0[58].xyz), vec3(CB0[59].xyz), vec3(CB0[60].xyz)) * f29;
    bvec3 f45 = bvec3(!(CB0[58].x == 2.0));
    vec3 f46 = vec4(f29, f43).xyz;
    vec3 f47 = textureLod(PrefilteredEnvIndoorTexture, f46, f43).xyz;
    vec3 f48;
    if (CB0[32].w == 0.0)
    {
        f48 = f47;
    ...
    {
        f48 = mix(f47, textureLod(PrefilteredEnvBlendTargetTexture, f46, f43).xyz, vec3(CB0[32].w));
    }
    vec4 f49 = texture(PrecomputedBRDFTexture, vec2(f27, max(9.9999997473787516355514526367188e-05, dot(f18, f25))));
    float f50 = f49.x;
    float f51 = f49.y;
    vec3 f52 = ((f31 * f50) + vec3(f51)) / vec3(f50 + f51);
    float f53 = 1.0 - f30;
    float f54 = f26 * f53;
    vec3 f55 = vec3(f53);
    vec3 f56 = f18 * f18;
    bvec3 f57 = lessThan(f18, vec3(0.0));
    ...
    float f65 = f58.z;
    vec3 f66 = -CB0[16].xyz;
    float f67 = dot(f18, f66) * ((1.0 - ((step(f41.x, f42) * clamp(CB0[29].z + (CB0[29].w * abs(f42 - 0.5)), 0.0, 1.0)) * f41.y)) * f39.y);
    vec3 f68 = normalize(f25 + f66);
    float f69 = clamp(f67, 0.0, 1.0);
    float f70 = f27 * f27;
    float f71 = max(0.001000000047497451305389404296875, dot(f18, f68));
    float f72 = dot(f66, f68);
    float f73 = 1.0 - f72;
    float f74 = f73 * f73;
    float f75 = (f74 * f74) * f73;
    vec3 f76 = vec3(f75) + (f31 * (1.0 - f75));
    float f77 = f70 * f70;
    float f78 = (((f71 * f77) - f71) * f71) + 1.0;
    vec3 f79 = (((((((f55 - (f52 * f54)) * (((((((CB0[40].xyz * f60) + (CB0[42].xyz * f61)) + (CB0[44].xyz * f62)) + (CB0[41].xyz * f63)) + (CB0[43].xyz * f64)) + (CB0[45].xyz * f65)) + (((((((CB0[34].xyz * f60) + (CB0[36].xyz * f61)) + (CB0[38].xyz * f62)) + (CB0[35].xyz * f63)) + (CB0[37].xyz * f64)) + (CB0[39].xyz * f65)) * f40))) * 1.0) + ((CB0[32].xyz + (CB0[33].xyz * f40)) * 1.0)) + ((((f55 - (f76 * f54)) * CB0[15].xyz) * f69) + (CB0[17].xyz * (f53 * clamp(-f67, 0.0, 1.0))))) + (f38.xyz * (f38.w * 120.0))) * f23.xyz) + (((mix(f48, textureLod(PrefilteredEnvTexture, vec4(vec3(f45.x ? f44.x : f29.x, f45.y ? f44.y : f29.y, f45.z ? f44.z : f29.z), f43).xyz, f43).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f29.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f40)) * f52) * f26) + (((f76 * (((f77 + (f77 * f77)) / (((f78 * f78) * ((f72 * 3.0) + 0.5)) * ((f71 * 0.75) + 0.25))) * f69)) * CB0[15].xyz) * 1.0));
    vec4 f80 = vec4(0.0);
    f80.x = f79.x;
    vec4 f81 = f80;
    f81.y = f79.y;
    ...
    vec4 f83 = f82;
    f83.w = VARYING2.w;
    float f84 = clamp(exp2((CB0[18].z * f24) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f85 = textureLod(PrefilteredEnvTexture, vec4(f28, 0.0).xyz, max(CB0[18].y, f84) * 5.0).xyz;
