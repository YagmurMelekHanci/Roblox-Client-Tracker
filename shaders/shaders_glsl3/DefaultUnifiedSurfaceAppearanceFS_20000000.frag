...
void main()
{
    ...
    vec4 f45 = texture(PrecomputedBRDFTexture, vec2(f25, max(9.9999997473787516355514526367188e-05, dot(f16, f23))));
    float f46 = f45.x;
    float f47 = f45.y;
    ...
    vec3 f51 = vec3(f49);
    vec3 f52 = f16 * f16;
    bvec3 f53 = lessThan(f16, vec3(0.0));
    ...
    float f61 = f54.z;
    vec3 f62 = -CB0[16].xyz;
    float f63 = dot(f16, f62) * ((1.0 - ((step(f39.x, f40) * clamp(CB0[29].z + (CB0[29].w * abs(f40 - 0.5)), 0.0, 1.0)) * f39.y)) * f37.y);
    vec3 f64 = normalize(f23 + f62);
    ...
    vec3 f72 = vec3(f71) + (f29 * (1.0 - f71));
    float f73 = f66 * f66;
    float f74 = (((f67 * f73) - f67) * f67) + 1.0;
    vec3 f75 = (((((((f51 - (f48 * f50)) * (((((((CB0[40].xyz * f56) + (CB0[42].xyz * f57)) + (CB0[44].xyz * f58)) + (CB0[41].xyz * f59)) + (CB0[43].xyz * f60)) + (CB0[45].xyz * f61)) + (((((((CB0[34].xyz * f56) + (CB0[36].xyz * f57)) + (CB0[38].xyz * f58)) + (CB0[35].xyz * f59)) + (CB0[37].xyz * f60)) + (CB0[39].xyz * f61)) * f38))) * 1.0) + ((CB0[32].xyz + (CB0[33].xyz * f38)) * 1.0)) + ((((f51 - (f72 * f50)) * CB0[15].xyz) * f65) + (CB0[17].xyz * (f49 * clamp(-f63, 0.0, 1.0))))) + (f36.xyz * (f36.w * 120.0))) * f21.xyz) + (((mix(f44, textureLod(PrefilteredEnvTexture, f42, f41).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f27.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f38)) * f48) * f24) + (((f72 * (((f73 + (f73 * f73)) / (((f74 * f74) * ((f68 * 3.0) + 0.5)) * ((f67 * 0.75) + 0.25))) * f65)) * CB0[15].xyz) * 1.0));
    vec4 f76 = vec4(0.0);
    f76.x = f75.x;
    vec4 f77 = f76;
    f77.y = f75.y;
