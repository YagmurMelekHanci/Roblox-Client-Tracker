...
void main()
{
    ...
    vec4 f37 = vec4(f19, 0.0) * mat4(CB0[0], CB0[1], CB0[2], CB0[3]);
    ...
    float f42 = 1.0 + clamp(0.0, f41, f36);
    vec4 f43 = VARYING8 + (f40 * f42);
    float f44 = f43.w;
    float f45 = f42 + clamp((texture(GBufferDepthTexture, f43.xy / vec2(f44)).x * 500.0) - f44, f41, f36);
    vec4 f46 = VARYING8 + (f40 * f45);
    float f47 = f46.w;
    float f48 = f45 + clamp((texture(GBufferDepthTexture, f46.xy / vec2(f47)).x * 500.0) - f47, f41, f36);
    vec4 f49 = VARYING8 + (f40 * f48);
    float f50 = f49.w;
    float f51 = f48 + clamp((texture(GBufferDepthTexture, f49.xy / vec2(f50)).x * 500.0) - f50, f41, f36);
    vec4 f52 = VARYING8 + (f40 * f51);
    float f53 = f52.w;
    float f54 = f51 + clamp((texture(GBufferDepthTexture, f52.xy / vec2(f53)).x * 500.0) - f53, f41, f36);
    vec4 f55 = VARYING8 + (f40 * f54);
    float f56 = f55.w;
    float f57 = f54 + clamp((texture(GBufferDepthTexture, f55.xy / vec2(f56)).x * 500.0) - f56, f41, f36);
    vec4 f58 = VARYING8 + (f40 * f57);
    float f59 = f58.w;
    float f60 = f57 + clamp((texture(GBufferDepthTexture, f58.xy / vec2(f59)).x * 500.0) - f59, f41, f36);
    vec4 f61 = VARYING8 + (f40 * f60);
    float f62 = f61.w;
    vec4 f63 = VARYING8 + (f40 * (f60 + clamp((texture(GBufferDepthTexture, f61.xy / vec2(f62)).x * 500.0) - f62, f41, f36)));
    float f64 = f63.w;
    vec2 f65 = f63.xy / vec2(f64);
    vec3 f66 = texture(GBufferColorTexture, f65).xyz;
    vec3 f67 = mat3(vec3(CB0[58].xyz), vec3(CB0[59].xyz), vec3(CB0[60].xyz)) * f19;
    bvec3 f68 = bvec3(!(CB0[58].x == 2.0));
    vec3 f69 = (textureLod(PrefilteredEnvTexture, vec4(vec3(f68.x ? f67.x : f19.x, f68.y ? f67.y : f19.y, f68.z ? f67.z : f19.z), 0.0).xyz, 0.0).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f19.y * 1.58823525905609130859375, 0.0, 1.0)))) * f6;
    vec3 f70 = vec3(clamp((length((CB0[11].xyz - VARYING7.xyz) - CB0[11].xyz) * CB0[52].y) - (CB0[51].z * CB0[52].y), 0.0, 1.0));
    float f71 = 0.08900000154972076416015625 + (f12 * 0.9110000133514404296875);
    vec3 f72 = -CB0[16].xyz;
    vec3 f73 = normalize(f17 + f72);
    float f74 = f71 * f71;
    ...
    vec3 f82 = mix(mix(mix((f35 * f35) * CB0[20].x, f30, vec3(clamp(clamp(((mix(texture(GBufferDepthTexture, f31).x * 500.0, f33, f34) - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), f30, f70), mix(mix(f69, (f66 * f66) * CB0[20].x, vec3((((float(abs(f65.x - 0.5) < 0.550000011920928955078125) * float(abs(f65.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f64) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f65).x * 500.0) - f64) < 10.0)) * float(f37.w > 0.0))), f69, f70) + (f5 * 0.100000001490116119384765625), vec3((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f16, f17))), 0.0, 1.0) + 0.300000011920928955078125) * (VARYING0.w * CB3[2].z))) + (((((vec3(f79) + (vec3(0.0199999995529651641845703125) * (1.0 - f79))) * (((f80 + (f80 * f80)) / (((f81 * f81) * ((f76 * 3.0) + 0.5)) * ((f75 * 0.75) + 0.25))) * clamp(dot(f15, f72) * f7, 0.0, 1.0))) * CB0[15].xyz) * 1.0) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f83 = vec4(0.0);
    f83.x = f82.x;
    vec4 f84 = f83;
    f84.y = f82.y;
    ...
    vec4 f86 = f85;
    f86.w = 1.0;
    vec3 f87 = textureLod(PrefilteredEnvTexture, vec4(f18, 0.0).xyz, max(CB0[18].y, VARYING6.w) * 5.0).xyz;
    bvec3 f88 = bvec3(!(CB0[18].w == 0.0));
    vec3 f89 = mix(vec3(f88.x ? CB0[19].xyz.x : f87.x, f88.y ? CB0[19].xyz.y : f87.y, f88.z ? CB0[19].xyz.z : f87.z), f86.xyz, vec3(VARYING6.w));
    vec4 f90 = f86;
    f90.x = f89.x;
    vec4 f91 = f90;
    f91.y = f89.y;
