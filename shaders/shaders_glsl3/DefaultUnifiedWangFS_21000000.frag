...
void main()
{
    ...
    float f49 = f39.z;
    float f50 = f34 * 5.0;
    vec3 f51 = mat3(vec3(CB0[58].xyz), vec3(CB0[59].xyz), vec3(CB0[60].xyz)) * f36;
    bvec3 f52 = bvec3(!(CB0[58].x == 2.0));
    vec3 f53 = vec4(f36, f50).xyz;
    vec3 f54 = textureLod(PrefilteredEnvIndoorTexture, f53, f50).xyz;
    vec3 f55;
    if (CB0[32].w == 0.0)
    {
        f55 = f54;
    ...
    {
        f55 = mix(f54, textureLod(PrefilteredEnvBlendTargetTexture, f53, f50).xyz, vec3(CB0[32].w));
    }
    vec4 f56 = texture(PrecomputedBRDFTexture, vec2(f34, max(9.9999997473787516355514526367188e-05, dot(f25, f32))));
    float f57 = f56.x;
    float f58 = f56.y;
    vec3 f59 = ((f38 * f57) + vec3(f58)) / vec3(f57 + f58);
    float f60 = 1.0 - f37;
    float f61 = f33 * f60;
    vec3 f62 = vec3(f60);
    vec3 f63 = f25 * f25;
    bvec3 f64 = lessThan(f25, vec3(0.0));
    ...
    float f72 = f65.z;
    vec3 f73 = -CB0[16].xyz;
    float f74 = dot(f25, f73) * ((1.0 - ((step(f48.x, f49) * clamp(CB0[29].z + (CB0[29].w * abs(f49 - 0.5)), 0.0, 1.0)) * f48.y)) * f46.y);
    vec3 f75 = normalize(f32 + f73);
    float f76 = clamp(f74, 0.0, 1.0);
    float f77 = f34 * f34;
    float f78 = max(0.001000000047497451305389404296875, dot(f25, f75));
    float f79 = dot(f73, f75);
    float f80 = 1.0 - f79;
    float f81 = f80 * f80;
    float f82 = (f81 * f81) * f80;
    vec3 f83 = vec3(f82) + (f38 * (1.0 - f82));
    float f84 = f77 * f77;
    float f85 = (((f78 * f84) - f78) * f78) + 1.0;
    vec3 f86 = (((((((f62 - (f59 * f61)) * (((((((CB0[40].xyz * f67) + (CB0[42].xyz * f68)) + (CB0[44].xyz * f69)) + (CB0[41].xyz * f70)) + (CB0[43].xyz * f71)) + (CB0[45].xyz * f72)) + (((((((CB0[34].xyz * f67) + (CB0[36].xyz * f68)) + (CB0[38].xyz * f69)) + (CB0[35].xyz * f70)) + (CB0[37].xyz * f71)) + (CB0[39].xyz * f72)) * f47))) * 1.0) + ((CB0[32].xyz + (CB0[33].xyz * f47)) * 1.0)) + ((((f62 - (f83 * f61)) * CB0[15].xyz) * f76) + (CB0[17].xyz * (f60 * clamp(-f74, 0.0, 1.0))))) + (f45.xyz * (f45.w * 120.0))) * f30.xyz) + (((mix(f55, textureLod(PrefilteredEnvTexture, vec4(vec3(f52.x ? f51.x : f36.x, f52.y ? f51.y : f36.y, f52.z ? f51.z : f36.z), f50).xyz, f50).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f36.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f47)) * f59) * f33) + (((f83 * (((f84 + (f84 * f84)) / (((f85 * f85) * ((f79 * 3.0) + 0.5)) * ((f78 * 0.75) + 0.25))) * f76)) * CB0[15].xyz) * 1.0));
    vec4 f87 = vec4(0.0);
    f87.x = f86.x;
    vec4 f88 = f87;
    f88.y = f86.y;
    ...
    vec4 f90 = f89;
    f90.w = VARYING2.w;
    float f91 = clamp(exp2((CB0[18].z * f31) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f92 = textureLod(PrefilteredEnvTexture, vec4(f35, 0.0).xyz, max(CB0[18].y, f91) * 5.0).xyz;
