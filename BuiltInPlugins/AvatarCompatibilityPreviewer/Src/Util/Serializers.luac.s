PROTO_0:
  LOADNIL R1
  GETTABLEKS R3 R0 K0 ["item"]
  GETTABLEKS R2 R3 K1 ["source"]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["SOURCE_BUILTIN"]
  JUMPIFNOTEQ R2 R3 [+32]
  DUPTABLE R2 K4 [{"source", "builtinItem"}]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["SOURCE_BUILTIN"]
  SETTABLEKS R3 R2 K1 ["source"]
  GETTABLEKS R5 R0 K5 ["palette"]
  GETTABLEKS R4 R5 K6 ["Serializer"]
  JUMPIFNOT R4 [+12]
  GETTABLEKS R5 R0 K5 ["palette"]
  GETTABLEKS R4 R5 K6 ["Serializer"]
  GETTABLEKS R3 R4 K7 ["serialize"]
  GETTABLEKS R5 R0 K0 ["item"]
  GETTABLEKS R4 R5 K3 ["builtinItem"]
  CALL R3 1 1
  JUMP [+4]
  GETTABLEKS R4 R0 K0 ["item"]
  GETTABLEKS R3 R4 K3 ["builtinItem"]
  SETTABLEKS R3 R2 K3 ["builtinItem"]
  MOVE R1 R2
  JUMP [+37]
  GETTABLEKS R3 R0 K0 ["item"]
  GETTABLEKS R2 R3 K1 ["source"]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K8 ["SOURCE_INSTANCE"]
  JUMPIFNOTEQ R2 R3 [+17]
  DUPTABLE R2 K10 [{"source", "key"}]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K8 ["SOURCE_INSTANCE"]
  SETTABLEKS R3 R2 K1 ["source"]
  GETTABLEKS R5 R0 K0 ["item"]
  GETTABLEKS R4 R5 K11 ["instance"]
  GETTABLEKS R3 R4 K12 ["Name"]
  SETTABLEKS R3 R2 K9 ["key"]
  MOVE R1 R2
  JUMP [+12]
  GETIMPORT R2 K14 [error]
  LOADK R4 K15 ["Unknown item source in serialization: %*"]
  GETTABLEKS R7 R0 K0 ["item"]
  GETTABLEKS R6 R7 K1 ["source"]
  NAMECALL R4 R4 K16 ["format"]
  CALL R4 2 1
  MOVE R3 R4
  CALL R2 1 0
  DUPTABLE R2 K18 [{"palette", "item", "isHidden"}]
  GETTABLEKS R4 R0 K5 ["palette"]
  GETTABLEKS R3 R4 K19 ["Key"]
  SETTABLEKS R3 R2 K5 ["palette"]
  SETTABLEKS R1 R2 K0 ["item"]
  GETTABLEKS R3 R0 K17 ["isHidden"]
  SETTABLEKS R3 R2 K17 ["isHidden"]
  RETURN R2 1

PROTO_1:
  GETUPVAL R1 0
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K0 ["map"]
  MOVE R4 R0
  DUPCLOSURE R5 K1 [PROTO_0]
  CAPTURE UPVAL U2
  CALL R3 2 -1
  NAMECALL R1 R1 K2 ["JSONEncode"]
  CALL R1 -1 -1
  RETURN R1 -1

PROTO_2:
  GETUPVAL R2 0
  GETTABLEKS R3 R1 K0 ["palette"]
  CALL R2 1 1
  JUMPIFNOTEQKNIL R2 [+3]
  LOADNIL R3
  RETURN R3 1
  JUMPIFNOTEQKNIL R2 [+2]
  LOADB R4 0 +1
  LOADB R4 1
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K1 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R4 R5 [+3]
  GETIMPORT R3 K3 [assert]
  CALL R3 2 0
  LOADNIL R3
  GETTABLEKS R5 R1 K4 ["item"]
  GETTABLEKS R4 R5 K5 ["source"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K6 ["SOURCE_BUILTIN"]
  JUMPIFNOTEQ R4 R5 [+28]
  DUPTABLE R4 K8 [{"source", "builtinItem"}]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K6 ["SOURCE_BUILTIN"]
  SETTABLEKS R5 R4 K5 ["source"]
  GETTABLEKS R6 R2 K9 ["Serializer"]
  JUMPIFNOT R6 [+10]
  GETTABLEKS R6 R2 K9 ["Serializer"]
  GETTABLEKS R5 R6 K10 ["deserialize"]
  GETTABLEKS R7 R1 K4 ["item"]
  GETTABLEKS R6 R7 K7 ["builtinItem"]
  CALL R5 1 1
  JUMP [+4]
  GETTABLEKS R6 R1 K4 ["item"]
  GETTABLEKS R5 R6 K7 ["builtinItem"]
  SETTABLEKS R5 R4 K7 ["builtinItem"]
  MOVE R3 R4
  JUMP [+43]
  GETTABLEKS R5 R1 K4 ["item"]
  GETTABLEKS R4 R5 K5 ["source"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K11 ["SOURCE_INSTANCE"]
  JUMPIFNOTEQ R4 R5 [+23]
  GETUPVAL R4 2
  GETTABLEKS R7 R1 K4 ["item"]
  GETTABLEKS R6 R7 K12 ["key"]
  NAMECALL R4 R4 K13 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOTEQKNIL R4 [+3]
  LOADNIL R5
  RETURN R5 1
  DUPTABLE R5 K15 [{"source", "instance"}]
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K11 ["SOURCE_INSTANCE"]
  SETTABLEKS R6 R5 K5 ["source"]
  SETTABLEKS R4 R5 K14 ["instance"]
  MOVE R3 R5
  JUMP [+12]
  GETIMPORT R4 K17 [error]
  LOADK R6 K18 ["Unknown item source in deserialization: %*"]
  GETTABLEKS R9 R1 K4 ["item"]
  GETTABLEKS R8 R9 K5 ["source"]
  NAMECALL R6 R6 K19 ["format"]
  CALL R6 2 1
  MOVE R5 R6
  CALL R4 1 0
  DUPTABLE R4 K21 [{"palette", "item", "isHidden"}]
  SETTABLEKS R2 R4 K0 ["palette"]
  SETTABLEKS R3 R4 K4 ["item"]
  GETTABLEKS R5 R1 K20 ["isHidden"]
  SETTABLEKS R5 R4 K20 ["isHidden"]
  RETURN R4 1

PROTO_3:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["JSONDecode"]
  CALL R1 2 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["collectArray"]
  MOVE R3 R1
  NEWCLOSURE R4 P0
  CAPTURE UPVAL U2
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CALL R2 2 -1
  RETURN R2 -1

PROTO_4:
  DUPTABLE R1 K2 [{"serialize", "deserialize"}]
  DUPCLOSURE R2 K3 [PROTO_1]
  CAPTURE UPVAL U0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  SETTABLEKS R2 R1 K0 ["serialize"]
  NEWCLOSURE R2 P1
  CAPTURE UPVAL U0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U3
  CAPTURE UPVAL U2
  CAPTURE VAL R0
  SETTABLEKS R2 R1 K1 ["deserialize"]
  RETURN R1 1

PROTO_5:
  NEWTABLE R1 0 0
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  FASTCALL1 TOSTRING R5 [+3]
  MOVE R8 R5
  GETIMPORT R7 K1 [tostring]
  CALL R7 1 1
  NEWTABLE R8 0 3
  GETTABLEKS R9 R6 K2 ["X"]
  GETTABLEKS R10 R6 K3 ["Y"]
  GETTABLEKS R11 R6 K4 ["Z"]
  SETLIST R8 R9 3 [1]
  SETTABLE R8 R1 R7
  FORGLOOP R2 2 [-17]
  RETURN R1 1

PROTO_6:
  NEWTABLE R1 0 0
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  FASTCALL1 TONUMBER R5 [+3]
  MOVE R8 R5
  GETIMPORT R7 K1 [tonumber]
  CALL R7 1 1
  FASTCALL1 TYPE R7 [+3]
  MOVE R9 R7
  GETIMPORT R8 K3 [type]
  CALL R8 1 1
  JUMPIFNOTEQKS R8 K4 ["number"] [+9]
  GETTABLEN R9 R6 1
  GETTABLEN R10 R6 2
  GETTABLEN R11 R6 3
  FASTCALL VECTOR [+2]
  GETIMPORT R8 K7 [Vector3.new]
  CALL R8 3 1
  SETTABLE R8 R1 R7
  FORGLOOP R2 2 [-21]
  RETURN R1 1

PROTO_7:
  NEWTABLE R1 0 0
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  NEWTABLE R7 0 0
  SETTABLE R7 R1 R5
  MOVE R7 R6
  LOADNIL R8
  LOADNIL R9
  FORGPREP R7
  JUMPIFNOTEQKS R10 K0 ["MeshOrigin"] [+18]
  FASTCALL1 TYPEOF R11 [+3]
  MOVE R13 R11
  GETIMPORT R12 K2 [typeof]
  CALL R12 1 1
  JUMPIFNOTEQKS R12 K3 ["CFrame"] [+11]
  GETTABLE R12 R1 R5
  NEWTABLE R13 0 1
  NAMECALL R14 R11 K4 ["GetComponents"]
  CALL R14 1 -1
  SETLIST R13 R14 -1 [1]
  SETTABLE R13 R12 R10
  JUMP [+10]
  JUMPIFNOTEQKS R10 K5 ["VertexData"] [+7]
  GETTABLE R12 R1 R5
  GETUPVAL R13 0
  MOVE R14 R11
  CALL R13 1 1
  SETTABLE R13 R12 R10
  JUMP [+2]
  GETTABLE R12 R1 R5
  SETTABLE R11 R12 R10
  FORGLOOP R7 2 [-30]
  FORGLOOP R2 2 [-39]
  GETUPVAL R2 1
  MOVE R4 R1
  NAMECALL R2 R2 K6 ["JSONEncode"]
  CALL R2 2 -1
  RETURN R2 -1

PROTO_8:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["JSONDecode"]
  CALL R1 2 1
  MOVE R2 R1
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  GETTABLE R7 R1 R5
  GETIMPORT R8 K3 [CFrame.new]
  GETTABLE R11 R1 R5
  GETTABLEKS R10 R11 K4 ["MeshOrigin"]
  FASTCALL1 TABLE_UNPACK R10 [+2]
  GETIMPORT R9 K6 [unpack]
  CALL R9 1 -1
  CALL R8 -1 1
  SETTABLEKS R8 R7 K4 ["MeshOrigin"]
  GETUPVAL R7 1
  GETTABLEKS R8 R6 K7 ["VertexData"]
  CALL R7 1 1
  SETTABLEKS R7 R6 K7 ["VertexData"]
  FORGLOOP R2 2 [-20]
  RETURN R1 1

PROTO_9:
  MOVE R1 R0
  LOADNIL R2
  LOADNIL R3
  FORGPREP R1
  GETTABLEKS R10 R5 K0 ["ToolStateData"]
  GETTABLEKS R9 R10 K1 ["ControlPointLocationData"]
  GETTABLEKS R6 R9 K2 ["Positions"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R13 R5 K0 ["ToolStateData"]
  GETTABLEKS R12 R13 K1 ["ControlPointLocationData"]
  GETTABLEKS R11 R12 K2 ["Positions"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-11]
  GETTABLEKS R10 R5 K0 ["ToolStateData"]
  GETTABLEKS R9 R10 K3 ["ControlPointLocationDataBackup"]
  GETTABLEKS R6 R9 K2 ["Positions"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R13 R5 K0 ["ToolStateData"]
  GETTABLEKS R12 R13 K3 ["ControlPointLocationDataBackup"]
  GETTABLEKS R11 R12 K2 ["Positions"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-11]
  GETTABLEKS R6 R5 K4 ["VertexData"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R11 R5 K4 ["VertexData"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-7]
  FORGLOOP R1 2 [-56]
  RETURN R0 1

PROTO_10:
  DUPCLOSURE R2 K0 [PROTO_9]
  CAPTURE UPVAL U0
  GETUPVAL R3 1
  MOVE R4 R0
  CALL R3 1 1
  GETUPVAL R4 1
  MOVE R5 R1
  CALL R4 1 1
  DUPTABLE R5 K3 [{"past", "future"}]
  MOVE R6 R2
  MOVE R7 R3
  CALL R6 1 1
  SETTABLEKS R6 R5 K1 ["past"]
  MOVE R6 R2
  MOVE R7 R4
  CALL R6 1 1
  SETTABLEKS R6 R5 K2 ["future"]
  GETUPVAL R6 2
  MOVE R8 R5
  NAMECALL R6 R6 K4 ["JSONEncode"]
  CALL R6 2 -1
  RETURN R6 -1

PROTO_11:
  MOVE R1 R0
  LOADNIL R2
  LOADNIL R3
  FORGPREP R1
  GETTABLEKS R10 R5 K0 ["ToolStateData"]
  GETTABLEKS R9 R10 K1 ["ControlPointLocationData"]
  GETTABLEKS R6 R9 K2 ["Positions"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R13 R5 K0 ["ToolStateData"]
  GETTABLEKS R12 R13 K1 ["ControlPointLocationData"]
  GETTABLEKS R11 R12 K2 ["Positions"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-11]
  GETTABLEKS R10 R5 K0 ["ToolStateData"]
  GETTABLEKS R9 R10 K3 ["ControlPointLocationDataBackup"]
  GETTABLEKS R6 R9 K2 ["Positions"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R13 R5 K0 ["ToolStateData"]
  GETTABLEKS R12 R13 K3 ["ControlPointLocationDataBackup"]
  GETTABLEKS R11 R12 K2 ["Positions"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-11]
  GETTABLEKS R6 R5 K4 ["VertexData"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  GETTABLEKS R11 R5 K4 ["VertexData"]
  GETUPVAL R12 0
  MOVE R13 R10
  CALL R12 1 1
  SETTABLE R12 R11 R9
  FORGLOOP R6 2 [-7]
  FORGLOOP R1 2 [-56]
  RETURN R0 1

PROTO_12:
  DUPCLOSURE R1 K0 [PROTO_11]
  CAPTURE UPVAL U0
  GETUPVAL R2 1
  MOVE R4 R0
  NAMECALL R2 R2 K1 ["JSONDecode"]
  CALL R2 2 1
  GETTABLEKS R3 R2 K2 ["past"]
  GETTABLEKS R4 R2 K3 ["future"]
  MOVE R5 R1
  MOVE R6 R3
  CALL R5 1 1
  MOVE R6 R1
  MOVE R7 R4
  CALL R6 1 1
  RETURN R5 2

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [game]
  LOADK R2 K2 ["HttpService"]
  NAMECALL R0 R0 K3 ["GetService"]
  CALL R0 2 1
  GETIMPORT R1 K5 [script]
  LOADK R3 K6 ["AvatarCompatibilityPreviewer"]
  NAMECALL R1 R1 K7 ["FindFirstAncestor"]
  CALL R1 2 1
  GETIMPORT R2 K9 [require]
  GETTABLEKS R4 R1 K10 ["Packages"]
  GETTABLEKS R3 R4 K11 ["Dash"]
  CALL R2 1 1
  GETIMPORT R3 K9 [require]
  GETTABLEKS R5 R1 K10 ["Packages"]
  GETTABLEKS R4 R5 K12 ["Framework"]
  CALL R3 1 1
  GETIMPORT R4 K9 [require]
  GETTABLEKS R9 R1 K10 ["Packages"]
  GETTABLEKS R8 R9 K13 ["_Index"]
  GETTABLEKS R7 R8 K14 ["LuaMeshEditingModule"]
  GETTABLEKS R6 R7 K14 ["LuaMeshEditingModule"]
  GETTABLEKS R5 R6 K15 ["Types"]
  CALL R4 1 1
  GETIMPORT R5 K9 [require]
  GETTABLEKS R8 R1 K16 ["Src"]
  GETTABLEKS R7 R8 K17 ["Util"]
  GETTABLEKS R6 R7 K18 ["Constants"]
  CALL R5 1 1
  GETIMPORT R6 K9 [require]
  GETTABLEKS R8 R1 K16 ["Src"]
  GETTABLEKS R7 R8 K15 ["Types"]
  CALL R6 1 1
  GETIMPORT R7 K9 [require]
  GETTABLEKS R10 R1 K16 ["Src"]
  GETTABLEKS R9 R10 K17 ["Util"]
  GETTABLEKS R8 R9 K19 ["getPaletteFromKey"]
  CALL R7 1 1
  GETTABLEKS R9 R3 K17 ["Util"]
  GETTABLEKS R8 R9 K20 ["deepCopy"]
  NEWTABLE R9 0 0
  DUPCLOSURE R10 K21 [PROTO_4]
  CAPTURE VAL R0
  CAPTURE VAL R2
  CAPTURE VAL R5
  CAPTURE VAL R7
  SETTABLEKS R10 R9 K22 ["createEquippedItemsSerializer"]
  DUPCLOSURE R10 K23 [PROTO_5]
  DUPCLOSURE R11 K24 [PROTO_6]
  DUPTABLE R12 K27 [{"serialize", "deserialize"}]
  DUPCLOSURE R13 K28 [PROTO_7]
  CAPTURE VAL R10
  CAPTURE VAL R0
  SETTABLEKS R13 R12 K25 ["serialize"]
  DUPCLOSURE R13 K29 [PROTO_8]
  CAPTURE VAL R0
  CAPTURE VAL R11
  SETTABLEKS R13 R12 K26 ["deserialize"]
  SETTABLEKS R12 R9 K30 ["cageMeshesDataSerializer"]
  DUPTABLE R12 K27 [{"serialize", "deserialize"}]
  DUPCLOSURE R13 K31 [PROTO_10]
  CAPTURE VAL R10
  CAPTURE VAL R8
  CAPTURE VAL R0
  SETTABLEKS R13 R12 K25 ["serialize"]
  DUPCLOSURE R13 K32 [PROTO_12]
  CAPTURE VAL R11
  CAPTURE VAL R0
  SETTABLEKS R13 R12 K26 ["deserialize"]
  SETTABLEKS R12 R9 K33 ["cageHistorySerializer"]
  RETURN R9 1
