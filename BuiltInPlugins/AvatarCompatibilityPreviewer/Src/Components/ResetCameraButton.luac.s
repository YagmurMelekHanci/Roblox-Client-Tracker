PROTO_0:
  GETUPVAL R1 0
  LOADK R3 K0 ["ResetCameraButton"]
  NAMECALL R1 R1 K1 ["use"]
  CALL R1 2 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K2 ["useContext"]
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K3 ["Context"]
  CALL R2 1 1
  GETIMPORT R3 K6 [UDim.new]
  LOADN R4 0
  GETTABLEKS R5 R1 K7 ["Spacing"]
  CALL R3 2 1
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K8 ["createElement"]
  GETUPVAL R5 3
  DUPTABLE R6 K15 [{"AnchorPoint", "LayoutOrder", "OnClick", "Style", "Size", "ZIndex"}]
  GETIMPORT R7 K17 [Vector2.new]
  LOADN R8 1
  LOADN R9 0
  CALL R7 2 1
  SETTABLEKS R7 R6 K9 ["AnchorPoint"]
  GETTABLEKS R7 R0 K10 ["LayoutOrder"]
  SETTABLEKS R7 R6 K10 ["LayoutOrder"]
  GETTABLEKS R7 R2 K18 ["resetCamera"]
  SETTABLEKS R7 R6 K11 ["OnClick"]
  LOADK R7 K19 ["Round"]
  SETTABLEKS R7 R6 K12 ["Style"]
  GETTABLEKS R7 R1 K13 ["Size"]
  SETTABLEKS R7 R6 K13 ["Size"]
  LOADN R7 2
  SETTABLEKS R7 R6 K14 ["ZIndex"]
  DUPTABLE R7 K22 [{"Spacing", "Icon", "Tooltip"}]
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K8 ["createElement"]
  LOADK R9 K23 ["UIPadding"]
  DUPTABLE R10 K28 [{"PaddingLeft", "PaddingRight", "PaddingTop", "PaddingBottom"}]
  SETTABLEKS R3 R10 K24 ["PaddingLeft"]
  SETTABLEKS R3 R10 K25 ["PaddingRight"]
  SETTABLEKS R3 R10 K26 ["PaddingTop"]
  SETTABLEKS R3 R10 K27 ["PaddingBottom"]
  CALL R8 2 1
  SETTABLEKS R8 R7 K7 ["Spacing"]
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K8 ["createElement"]
  GETUPVAL R9 4
  DUPTABLE R10 K30 [{"Image"}]
  GETTABLEKS R11 R1 K29 ["Image"]
  SETTABLEKS R11 R10 K29 ["Image"]
  CALL R8 2 1
  SETTABLEKS R8 R7 K20 ["Icon"]
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K8 ["createElement"]
  GETUPVAL R9 5
  DUPTABLE R10 K32 [{"Text"}]
  LOADK R11 K33 ["Reset Camera"]
  SETTABLEKS R11 R10 K31 ["Text"]
  CALL R8 2 1
  SETTABLEKS R8 R7 K21 ["Tooltip"]
  CALL R4 3 -1
  RETURN R4 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AvatarCompatibilityPreviewer"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Framework"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R6 R0 K9 ["Src"]
  GETTABLEKS R5 R6 K10 ["Components"]
  GETTABLEKS R4 R5 K11 ["AvatarScreenContext"]
  CALL R3 1 1
  GETTABLEKS R4 R1 K12 ["UI"]
  GETTABLEKS R5 R4 K13 ["Button"]
  GETTABLEKS R6 R4 K14 ["Image"]
  GETTABLEKS R7 R4 K15 ["Tooltip"]
  GETTABLEKS R9 R1 K16 ["ContextServices"]
  GETTABLEKS R8 R9 K17 ["Stylizer"]
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K9 ["Src"]
  GETTABLEKS R11 R12 K18 ["Resources"]
  GETTABLEKS R10 R11 K19 ["Theme"]
  CALL R9 1 1
  DUPCLOSURE R10 K20 [PROTO_0]
  CAPTURE VAL R8
  CAPTURE VAL R2
  CAPTURE VAL R3
  CAPTURE VAL R5
  CAPTURE VAL R6
  CAPTURE VAL R7
  RETURN R10 1
