PROTO_0:
  LOADK R4 K0 ["BasePart"]
  NAMECALL R2 R0 K1 ["IsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+25]
  FASTCALL1 TYPEOF R1 [+3]
  MOVE R3 R1
  GETIMPORT R2 K3 [typeof]
  CALL R2 1 1
  JUMPIFNOTEQKS R2 K4 ["string"] [+8]
  GETIMPORT R2 K7 [Color3.fromHex]
  MOVE R3 R1
  CALL R2 1 1
  SETTABLEKS R2 R0 K8 ["Color"]
  RETURN R0 0
  GETIMPORT R2 K10 [Color3.fromRGB]
  GETTABLEKS R3 R1 K11 ["r"]
  GETTABLEKS R4 R1 K12 ["g"]
  GETTABLEKS R5 R1 K13 ["b"]
  CALL R2 3 1
  SETTABLEKS R2 R0 K8 ["Color"]
  RETURN R0 0

PROTO_1:
  JUMPIFNOT R0 [+52]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["get"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 1
  JUMPIFNOT R2 [+1]
  RETURN R2 1
  GETIMPORT R3 K2 [pcall]
  GETIMPORT R5 K4 [game]
  GETTABLEKS R4 R5 K5 ["GetService"]
  GETIMPORT R5 K4 [game]
  MOVE R6 R0
  CALL R3 3 2
  JUMPIFNOT R3 [+2]
  JUMPIFNOT R4 [+1]
  RETURN R4 1
  JUMPIFNOTEQKS R0 K6 ["workspace"] [+4]
  GETIMPORT R5 K7 [workspace]
  RETURN R5 1
  JUMPIFNOTEQKS R0 K8 ["StarterPlayerScripts"] [+12]
  GETIMPORT R5 K4 [game]
  LOADK R7 K9 ["StarterPlayer"]
  NAMECALL R5 R5 K5 ["GetService"]
  CALL R5 2 1
  LOADK R7 K8 ["StarterPlayerScripts"]
  NAMECALL R5 R5 K10 ["FindFirstChildWhichIsA"]
  CALL R5 2 -1
  RETURN R5 -1
  JUMPIFNOTEQKS R0 K11 ["StarterCharacterScripts"] [+12]
  GETIMPORT R5 K4 [game]
  LOADK R7 K9 ["StarterPlayer"]
  NAMECALL R5 R5 K5 ["GetService"]
  CALL R5 2 1
  LOADK R7 K11 ["StarterCharacterScripts"]
  NAMECALL R5 R5 K10 ["FindFirstChildWhichIsA"]
  CALL R5 2 -1
  RETURN R5 -1
  RETURN R0 0

PROTO_2:
  LOADNIL R1
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["position"]
  JUMPIFNOT R2 [+18]
  GETIMPORT R2 K3 [CFrame.new]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K0 ["position"]
  GETTABLEKS R4 R5 K5 ["x"]
  ORK R3 R4 K4 [0]
  LOADN R4 0
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K0 ["position"]
  GETTABLEKS R6 R7 K6 ["z"]
  ORK R5 R6 K4 [0]
  CALL R2 3 1
  MOVE R1 R2
  JUMP [+7]
  GETIMPORT R2 K3 [CFrame.new]
  LOADN R3 0
  LOADN R4 0
  LOADN R5 0
  CALL R2 3 1
  MOVE R1 R2
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K7 ["positionOrigin"]
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K8 ["get"]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K9 ["referenceInstance"]
  GETUPVAL R5 2
  CALL R3 2 1
  GETUPVAL R5 3
  GETTABLEKS R4 R5 K10 ["getWorldBoundingBox"]
  MOVE R5 R3
  CALL R4 1 2
  GETIMPORT R6 K3 [CFrame.new]
  CALL R6 0 1
  LOADNIL R7
  LOADNIL R8
  JUMPIFNOTEQKS R2 K11 ["absolute"] [+6]
  GETIMPORT R9 K3 [CFrame.new]
  CALL R9 0 1
  MOVE R6 R9
  JUMP [+40]
  JUMPIFNOTEQKS R2 K12 ["relative"] [+9]
  JUMPIFNOT R3 [+7]
  GETIMPORT R9 K3 [CFrame.new]
  GETTABLEKS R10 R4 K13 ["Position"]
  CALL R9 1 1
  MOVE R6 R9
  JUMP [+30]
  JUMPIFNOTEQKS R2 K14 ["onTop"] [+10]
  JUMPIFNOT R3 [+8]
  GETIMPORT R9 K3 [CFrame.new]
  GETTABLEKS R10 R4 K13 ["Position"]
  CALL R9 1 1
  MOVE R6 R9
  MOVE R7 R3
  JUMP [+19]
  GETUPVAL R9 4
  CALL R9 0 1
  JUMPIFNOT R9 [+9]
  JUMPIFNOTEQKS R2 K15 ["currentCamera"] [+8]
  GETUPVAL R10 3
  GETTABLEKS R9 R10 K16 ["getCurrentLookingCFrame"]
  CALL R9 0 5
  MOVE R6 R9
  MOVE R8 R10
  JUMP [+7]
  GETUPVAL R10 3
  GETTABLEKS R9 R10 K17 ["getRequestCFrame"]
  GETUPVAL R10 2
  CALL R9 1 2
  MOVE R6 R9
  MOVE R8 R10
  MOVE R11 R1
  NAMECALL R9 R6 K18 ["ToWorldSpace"]
  CALL R9 2 1
  GETUPVAL R11 3
  GETTABLEKS R10 R11 K19 ["getGroundLevelAt"]
  GETTABLEKS R11 R9 K13 ["Position"]
  MOVE R12 R7
  MOVE R13 R5
  MOVE R14 R8
  CALL R10 4 1
  GETTABLEKS R15 R9 K13 ["Position"]
  GETTABLEKS R14 R15 K20 ["Y"]
  MINUS R13 R14
  ADD R12 R13 R10
  NAMECALL R15 R0 K22 ["GetExtentsSize"]
  CALL R15 1 1
  GETTABLEKS R14 R15 K20 ["Y"]
  DIVK R13 R14 K21 [2]
  ADD R11 R12 R13
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K23 ["elevation"]
  JUMPIFNOT R12 [+4]
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K23 ["elevation"]
  ADD R11 R11 R12
  LOADN R13 0
  LOADN R15 0
  FASTCALL3 VECTOR R13 R11 R15
  MOVE R14 R11
  GETIMPORT R12 K25 [Vector3.new]
  CALL R12 3 1
  ADD R9 R9 R12
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K26 ["rotation"]
  JUMPIFNOT R12 [+18]
  GETIMPORT R13 K28 [CFrame.fromEulerAngles]
  LOADN R14 0
  GETUPVAL R17 0
  GETTABLEKS R16 R17 K26 ["rotation"]
  FASTCALL1 MATH_RAD R16 [+2]
  GETIMPORT R15 K31 [math.rad]
  CALL R15 1 1
  LOADN R16 0
  CALL R13 3 1
  GETTABLEKS R14 R9 K32 ["Rotation"]
  MUL R12 R13 R14
  GETTABLEKS R13 R9 K13 ["Position"]
  ADD R9 R12 R13
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K33 ["offsetCFrame"]
  JUMPIFNOT R12 [+4]
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K33 ["offsetCFrame"]
  MUL R9 R9 R12
  MOVE R14 R9
  NAMECALL R12 R0 K34 ["PivotTo"]
  CALL R12 2 0
  RETURN R0 0

PROTO_3:
  GETTABLEKS R1 R0 K0 ["requestId"]
  GETTABLEKS R2 R0 K1 ["arguments"]
  GETTABLEKS R3 R2 K2 ["direct_instance"]
  GETTABLEKS R4 R2 K3 ["newInstanceId"]
  JUMPIF R4 [+2]
  GETTABLEKS R4 R2 K4 ["instanceId"]
  MOVE R5 R3
  JUMPIF R5 [+6]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K5 ["get"]
  MOVE R6 R4
  MOVE R7 R1
  CALL R5 2 1
  JUMPIF R5 [+4]
  GETIMPORT R6 K7 [error]
  LOADK R7 K8 ["Failed to find instance to set properties on"]
  CALL R6 1 0
  FASTCALL2K ASSERT R5 K9 [+5]
  MOVE R7 R5
  LOADK R8 K9 ["Luau"]
  GETIMPORT R6 K11 [assert]
  CALL R6 2 0
  JUMPIF R3 [+5]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K12 ["StartRecordingActions"]
  MOVE R7 R1
  CALL R6 1 0
  GETTABLEKS R6 R2 K13 ["properties"]
  JUMPIF R6 [+2]
  NEWTABLE R6 0 0
  GETTABLEKS R7 R6 K14 ["instanceName"]
  JUMPIFNOT R7 [+4]
  GETTABLEKS R7 R6 K14 ["instanceName"]
  SETTABLEKS R7 R5 K15 ["Name"]
  GETTABLEKS R7 R6 K16 ["size"]
  JUMPIFNOT R7 [+23]
  LOADK R9 K17 ["BasePart"]
  NAMECALL R7 R5 K18 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+18]
  GETTABLEKS R9 R6 K16 ["size"]
  GETTABLEKS R8 R9 K19 ["x"]
  GETTABLEKS R10 R6 K16 ["size"]
  GETTABLEKS R9 R10 K20 ["y"]
  GETTABLEKS R11 R6 K16 ["size"]
  GETTABLEKS R10 R11 K21 ["z"]
  FASTCALL VECTOR [+2]
  GETIMPORT R7 K24 [Vector3.new]
  CALL R7 3 1
  SETTABLEKS R7 R5 K25 ["Size"]
  LOADK R9 K26 ["Model"]
  NAMECALL R7 R5 K18 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+84]
  GETTABLEKS R7 R6 K27 ["scale"]
  JUMPIFNOT R7 [+15]
  GETTABLEKS R7 R6 K27 ["scale"]
  LOADN R8 0
  JUMPIFNOTLT R8 R7 [+11]
  NAMECALL R7 R5 K28 ["GetScale"]
  CALL R7 1 1
  GETTABLEKS R11 R6 K27 ["scale"]
  MUL R10 R7 R11
  NAMECALL R8 R5 K29 ["ScaleTo"]
  CALL R8 2 0
  JUMP [+66]
  GETTABLEKS R7 R6 K30 ["height"]
  JUMPIFNOT R7 [+25]
  GETTABLEKS R7 R6 K30 ["height"]
  LOADK R8 K31 [0.1]
  JUMPIFNOTLT R8 R7 [+21]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K32 ["getWorldBoundingBox"]
  MOVE R8 R5
  CALL R7 1 2
  GETTABLEKS R9 R8 K33 ["Y"]
  LOADK R10 K31 [0.1]
  JUMPIFNOTLT R10 R9 [+49]
  GETTABLEKS R10 R6 K30 ["height"]
  GETTABLEKS R11 R8 K33 ["Y"]
  DIV R9 R10 R11
  MOVE R12 R9
  NAMECALL R10 R5 K29 ["ScaleTo"]
  CALL R10 2 0
  JUMP [+38]
  GETTABLEKS R7 R6 K16 ["size"]
  JUMPIFNOT R7 [+35]
  GETTABLEKS R8 R6 K16 ["size"]
  GETTABLEKS R7 R8 K20 ["y"]
  LOADK R8 K31 [0.1]
  JUMPIFNOTLT R8 R7 [+29]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K32 ["getWorldBoundingBox"]
  MOVE R8 R5
  CALL R7 1 2
  GETTABLEKS R9 R8 K33 ["Y"]
  LOADK R10 K31 [0.1]
  JUMPIFNOTLT R10 R9 [+19]
  GETTABLEKS R10 R6 K16 ["size"]
  GETTABLEKS R9 R10 K20 ["y"]
  LOADK R10 K31 [0.1]
  JUMPIFNOTLT R10 R9 [+12]
  GETTABLEKS R11 R6 K16 ["size"]
  GETTABLEKS R10 R11 K20 ["y"]
  GETTABLEKS R11 R8 K33 ["Y"]
  DIV R9 R10 R11
  MOVE R12 R9
  NAMECALL R10 R5 K29 ["ScaleTo"]
  CALL R10 2 0
  GETTABLEKS R7 R6 K34 ["source"]
  JUMPIFNOT R7 [+34]
  LOADK R9 K35 ["Script"]
  NAMECALL R7 R5 K18 ["IsA"]
  CALL R7 2 1
  JUMPIF R7 [+5]
  LOADK R9 K36 ["ModuleScript"]
  NAMECALL R7 R5 K18 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+24]
  GETUPVAL R7 2
  LOADK R9 K37 ["UpdateScriptSource"]
  DUPTABLE R10 K40 [{"requestId", "instanceId", "script", "source", "isNewScript"}]
  SETTABLEKS R1 R10 K0 ["requestId"]
  SETTABLEKS R4 R10 K4 ["instanceId"]
  SETTABLEKS R5 R10 K38 ["script"]
  GETTABLEKS R11 R6 K34 ["source"]
  SETTABLEKS R11 R10 K34 ["source"]
  GETTABLEKS R12 R2 K3 ["newInstanceId"]
  JUMPIFEQ R4 R12 [+2]
  LOADB R11 0 +1
  LOADB R11 1
  SETTABLEKS R11 R10 K39 ["isNewScript"]
  NAMECALL R7 R7 K41 ["DisplayContent"]
  CALL R7 3 0
  GETTABLEKS R7 R6 K42 ["color"]
  JUMPIFNOT R7 [+70]
  GETTABLEKS R7 R6 K42 ["color"]
  LOADK R10 K17 ["BasePart"]
  NAMECALL R8 R5 K18 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+25]
  FASTCALL1 TYPEOF R7 [+3]
  MOVE R9 R7
  GETIMPORT R8 K44 [typeof]
  CALL R8 1 1
  JUMPIFNOTEQKS R8 K45 ["string"] [+8]
  GETIMPORT R8 K48 [Color3.fromHex]
  MOVE R9 R7
  CALL R8 1 1
  SETTABLEKS R8 R5 K49 ["Color"]
  JUMP [+11]
  GETIMPORT R8 K51 [Color3.fromRGB]
  GETTABLEKS R9 R7 K52 ["r"]
  GETTABLEKS R10 R7 K53 ["g"]
  GETTABLEKS R11 R7 K54 ["b"]
  CALL R8 3 1
  SETTABLEKS R8 R5 K49 ["Color"]
  NAMECALL R7 R5 K55 ["GetDescendants"]
  CALL R7 1 3
  FORGPREP R7
  GETTABLEKS R12 R6 K42 ["color"]
  LOADK R15 K17 ["BasePart"]
  NAMECALL R13 R11 K18 ["IsA"]
  CALL R13 2 1
  JUMPIFNOT R13 [+25]
  FASTCALL1 TYPEOF R12 [+3]
  MOVE R14 R12
  GETIMPORT R13 K44 [typeof]
  CALL R13 1 1
  JUMPIFNOTEQKS R13 K45 ["string"] [+8]
  GETIMPORT R13 K48 [Color3.fromHex]
  MOVE R14 R12
  CALL R13 1 1
  SETTABLEKS R13 R11 K49 ["Color"]
  JUMP [+11]
  GETIMPORT R13 K51 [Color3.fromRGB]
  GETTABLEKS R14 R12 K52 ["r"]
  GETTABLEKS R15 R12 K53 ["g"]
  GETTABLEKS R16 R12 K54 ["b"]
  CALL R13 3 1
  SETTABLEKS R13 R11 K49 ["Color"]
  FORGLOOP R7 2 [-33]
  GETTABLEKS R7 R6 K56 ["material"]
  JUMPIFNOT R7 [+12]
  LOADK R9 K17 ["BasePart"]
  NAMECALL R7 R5 K18 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+7]
  GETIMPORT R8 K59 [Enum.Material]
  GETTABLEKS R9 R6 K56 ["material"]
  GETTABLE R7 R8 R9
  SETTABLEKS R7 R5 K58 ["Material"]
  MOVE R7 R3
  JUMPIF R7 [+11]
  GETTABLEKS R7 R6 K60 ["position"]
  JUMPIF R7 [+8]
  GETTABLEKS R7 R6 K61 ["elevation"]
  JUMPIF R7 [+5]
  GETTABLEKS R7 R6 K62 ["rotation"]
  JUMPIF R7 [+2]
  GETTABLEKS R7 R6 K63 ["referenceInstance"]
  JUMPIFNOT R7 [+11]
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K32 ["getWorldBoundingBox"]
  MOVE R9 R5
  NEWCLOSURE R10 P0
  CAPTURE VAL R6
  CAPTURE UPVAL U0
  CAPTURE VAL R1
  CAPTURE UPVAL U1
  CAPTURE UPVAL U3
  CALL R8 2 0
  LOADNIL R8
  GETUPVAL R9 4
  CALL R9 0 1
  JUMPIFNOT R9 [+10]
  GETTABLEKS R9 R6 K64 ["parentInstance"]
  JUMPIF R9 [+5]
  GETUPVAL R9 5
  GETTABLEKS R10 R6 K65 ["parentInstanceId"]
  MOVE R11 R1
  CALL R9 2 1
  MOVE R8 R9
  JUMP [+6]
  GETUPVAL R9 5
  GETTABLEKS R10 R6 K65 ["parentInstanceId"]
  MOVE R11 R1
  CALL R9 2 1
  MOVE R8 R9
  JUMPIFNOT R8 [+2]
  SETTABLEKS R8 R5 K66 ["Parent"]
  JUMPIF R3 [+5]
  GETUPVAL R10 0
  GETTABLEKS R9 R10 K67 ["EndRecordingActions"]
  MOVE R10 R1
  CALL R9 1 0
  GETUPVAL R10 6
  GETTABLEKS R9 R10 K68 ["getInstanceTypes"]
  CALL R9 0 1
  GETUPVAL R11 6
  GETTABLEKS R10 R11 K69 ["registerInstanceV2"]
  MOVE R11 R5
  MOVE R12 R1
  GETTABLEKS R13 R9 K70 ["Created"]
  CALL R10 3 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["StreamingServiceDispatcherRegistry"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K7 ["Commands"]
  GETTABLEKS R3 R4 K8 ["BuilderCommands"]
  GETTABLEKS R2 R3 K9 ["BuilderNameMap"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K10 ["Utils"]
  GETTABLEKS R3 R4 K11 ["CliAdapter"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R6 R0 K6 ["Src"]
  GETTABLEKS R5 R6 K12 ["ContextCollectors"]
  GETTABLEKS R4 R5 K13 ["RecentInstancesContext"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R6 R0 K6 ["Src"]
  GETTABLEKS R5 R6 K14 ["Types"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K6 ["Src"]
  GETTABLEKS R7 R8 K10 ["Utils"]
  GETTABLEKS R6 R7 K10 ["Utils"]
  CALL R5 1 1
  GETTABLEKS R6 R2 K15 ["GetService"]
  LOADK R7 K16 ["ChatbotUIService"]
  CALL R6 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R11 R0 K6 ["Src"]
  GETTABLEKS R10 R11 K17 ["Flags"]
  GETTABLEKS R9 R10 K18 ["FFlagConvAIAddCommandSearchInsertAsset"]
  CALL R8 1 1
  GETTABLEKS R7 R8 K19 ["Get"]
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K6 ["Src"]
  GETTABLEKS R11 R12 K17 ["Flags"]
  GETTABLEKS R10 R11 K20 ["FFlagFixCurrentCameraPlacement"]
  CALL R9 1 1
  GETTABLEKS R8 R9 K19 ["Get"]
  DUPCLOSURE R9 K21 [PROTO_0]
  DUPCLOSURE R10 K22 [PROTO_1]
  CAPTURE VAL R1
  DUPCLOSURE R11 K23 [PROTO_3]
  CAPTURE VAL R1
  CAPTURE VAL R5
  CAPTURE VAL R6
  CAPTURE VAL R8
  CAPTURE VAL R7
  CAPTURE VAL R10
  CAPTURE VAL R3
  RETURN R11 1
