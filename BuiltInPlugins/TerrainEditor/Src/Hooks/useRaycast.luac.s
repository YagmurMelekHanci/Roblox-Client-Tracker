PROTO_0:
  GETUPVAL R0 0
  NAMECALL R0 R0 K0 ["IsMouseDown"]
  CALL R0 1 1
  JUMPIF R0 [+15]
  GETIMPORT R0 K3 [CFrame.lookAt]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K4 ["Position"]
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K4 ["Position"]
  GETUPVAL R4 2
  CALL R4 0 1
  ADD R2 R3 R4
  CALL R0 2 1
  GETUPVAL R1 3
  MOVE R2 R0
  CALL R1 1 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R7 0
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K1 ["BrushSettings"]
  GETTABLE R6 R7 R8
  GETUPVAL R8 2
  GETTABLEKS R7 R8 K2 ["BrushSize"]
  GETTABLE R5 R6 R7
  GETTABLEKS R4 R5 K3 ["Size"]
  DIVK R3 R4 K0 [2]
  GETUPVAL R5 3
  GETTABLEKS R4 R5 K4 ["VoxelResolution"]
  MUL R2 R3 R4
  GETUPVAL R4 3
  GETTABLEKS R3 R4 K5 ["TerrainCacheMultiplier"]
  MUL R1 R2 R3
  GETUPVAL R3 3
  GETTABLEKS R2 R3 K6 ["TerrainCacheMaxRadius"]
  FASTCALL2 MATH_MIN R1 R2 [+3]
  GETIMPORT R0 K9 [math.min]
  CALL R0 2 1
  RETURN R0 1

PROTO_2:
  GETUPVAL R0 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["Draw"]
  JUMPIFNOTEQ R0 R1 [+11]
  GETUPVAL R2 2
  GETUPVAL R4 3
  GETTABLEKS R3 R4 K1 ["BrushSettings"]
  GETTABLE R1 R2 R3
  GETUPVAL R3 4
  GETTABLEKS R2 R3 K2 ["TemporarySmooth"]
  GETTABLE R0 R1 R2
  JUMPIFNOT R0 [+26]
  GETUPVAL R0 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K3 ["Paint"]
  JUMPIFNOTEQ R0 R1 [+38]
  GETUPVAL R0 5
  JUMPIFNOT R0 [+35]
  GETUPVAL R1 5
  GETUPVAL R3 6
  GETTABLEKS R2 R3 K4 ["TargetMaterial"]
  GETTABLE R0 R1 R2
  GETIMPORT R1 K8 [Enum.Material.Air]
  JUMPIFEQ R0 R1 [+10]
  GETUPVAL R1 5
  GETUPVAL R3 6
  GETTABLEKS R2 R3 K9 ["SourceMaterial"]
  GETTABLE R0 R1 R2
  GETIMPORT R1 K8 [Enum.Material.Air]
  JUMPIFNOTEQ R0 R1 [+18]
  GETUPVAL R3 2
  GETUPVAL R5 3
  GETTABLEKS R4 R5 K1 ["BrushSettings"]
  GETTABLE R2 R3 R4
  GETUPVAL R4 4
  GETTABLEKS R3 R4 K10 ["PlaneLock"]
  GETTABLE R1 R2 R3
  GETUPVAL R3 7
  GETTABLEKS R2 R3 K11 ["Off"]
  JUMPIFEQ R1 R2 [+2]
  LOADB R0 0 +1
  LOADB R0 1
  RETURN R0 1
  LOADB R0 0
  RETURN R0 1

PROTO_3:
  GETUPVAL R3 0
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K0 ["BrushSettings"]
  GETTABLE R2 R3 R4
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K1 ["BrushShape"]
  GETTABLE R1 R2 R3
  GETUPVAL R4 0
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K0 ["BrushSettings"]
  GETTABLE R3 R4 R5
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K2 ["BrushSize"]
  GETTABLE R2 R3 R4
  GETTABLEKS R4 R2 K3 ["Size"]
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K4 ["VoxelResolution"]
  MUL R3 R4 R5
  GETUPVAL R6 4
  GETTABLEKS R5 R6 K5 ["Sphere"]
  JUMPIFNOTEQ R1 R5 [+3]
  MOVE R4 R3
  JUMP [+6]
  GETTABLEKS R5 R2 K6 ["Height"]
  GETUPVAL R7 3
  GETTABLEKS R6 R7 K4 ["VoxelResolution"]
  MUL R4 R5 R6
  GETUPVAL R5 5
  GETUPVAL R8 0
  GETUPVAL R10 1
  GETTABLEKS R9 R10 K0 ["BrushSettings"]
  GETTABLE R7 R8 R9
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K7 ["PivotPosition"]
  GETTABLE R6 R7 R8
  GETUPVAL R8 6
  GETTABLEKS R7 R8 K8 ["current"]
  MOVE R8 R4
  CALL R5 3 1
  GETUPVAL R6 7
  LOADK R8 K9 ["BrushVisual"]
  NAMECALL R6 R6 K10 ["FindFirstChild"]
  CALL R6 2 1
  JUMPIF R6 [+1]
  RETURN R0 0
  LOADK R9 K11 ["Central"]
  LOADB R10 1
  NAMECALL R7 R6 K10 ["FindFirstChild"]
  CALL R7 3 1
  JUMPIFNOT R7 [+33]
  GETUPVAL R10 4
  GETTABLEKS R9 R10 K12 ["Cylinder"]
  JUMPIFNOTEQ R1 R9 [+9]
  GETIMPORT R9 K15 [CFrame.Angles]
  LOADK R10 K16 [1.5707963267949]
  LOADN R11 0
  LOADN R12 0
  CALL R9 3 1
  ADD R8 R9 R5
  JUMP [+4]
  GETIMPORT R8 K18 [CFrame.new]
  MOVE R9 R5
  CALL R8 1 1
  SETTABLEKS R8 R7 K13 ["CFrame"]
  LOADK R11 K19 ["OnTop"]
  LOADB R12 1
  NAMECALL R9 R6 K10 ["FindFirstChild"]
  CALL R9 3 1
  SETTABLEKS R8 R9 K13 ["CFrame"]
  LOADK R11 K20 ["Outline"]
  LOADB R12 1
  NAMECALL R9 R6 K10 ["FindFirstChild"]
  CALL R9 3 1
  SETTABLEKS R8 R9 K13 ["CFrame"]
  RETURN R0 0

PROTO_4:
  NEWTABLE R1 1 0
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["BrushSettings"]
  NEWTABLE R3 1 0
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["State"]
  SETTABLE R0 R3 R4
  SETTABLE R3 R1 R2
  GETUPVAL R2 2
  GETUPVAL R4 3
  GETTABLEKS R3 R4 K2 ["Build"]
  JUMPIFEQ R2 R3 [+4]
  GETUPVAL R2 4
  MOVE R3 R0
  CALL R2 1 0
  GETUPVAL R2 5
  MOVE R3 R0
  CALL R2 1 0
  GETUPVAL R2 6
  MOVE R3 R1
  CALL R2 1 0
  RETURN R0 0

PROTO_5:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["join"]
  MOVE R2 R0
  DUPTABLE R3 K3 [{"CacheID", "CachePoint"}]
  GETUPVAL R4 1
  SETTABLEKS R4 R3 K1 ["CacheID"]
  GETUPVAL R4 2
  SETTABLEKS R4 R3 K2 ["CachePoint"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_6:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["Origin"]
  GETTABLEKS R0 R1 K1 ["Position"]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K2 ["UnitRay"]
  GETTABLEKS R1 R2 K3 ["Direction"]
  GETIMPORT R2 K6 [RaycastParams.new]
  CALL R2 0 1
  GETUPVAL R5 1
  GETUPVAL R7 2
  GETTABLEKS R6 R7 K7 ["BrushSettings"]
  GETTABLE R4 R5 R6
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K8 ["IgnoreParts"]
  GETTABLE R3 R4 R5
  JUMPIFNOT R3 [+25]
  NEWTABLE R3 0 1
  GETUPVAL R4 4
  SETLIST R3 R4 1 [1]
  GETUPVAL R4 5
  LOADK R6 K9 ["Baseplate"]
  NAMECALL R4 R4 K10 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  FASTCALL2 TABLE_INSERT R3 R4 [+5]
  MOVE R6 R3
  MOVE R7 R4
  GETIMPORT R5 K13 [table.insert]
  CALL R5 2 0
  GETIMPORT R5 K17 [Enum.RaycastFilterType.Whitelist]
  SETTABLEKS R5 R2 K18 ["FilterType"]
  SETTABLEKS R3 R2 K19 ["FilterDescendantsInstances"]
  JUMP [+41]
  NEWTABLE R3 0 0
  GETUPVAL R5 6
  GETTABLEKS R4 R5 K20 ["LocalPlayer"]
  JUMPIFNOT R4 [+17]
  GETUPVAL R6 6
  GETTABLEKS R5 R6 K20 ["LocalPlayer"]
  GETTABLEKS R4 R5 K21 ["Character"]
  JUMPIFNOT R4 [+11]
  GETUPVAL R8 6
  GETTABLEKS R7 R8 K20 ["LocalPlayer"]
  GETTABLEKS R6 R7 K21 ["Character"]
  FASTCALL2 TABLE_INSERT R3 R6 [+4]
  MOVE R5 R3
  GETIMPORT R4 K13 [table.insert]
  CALL R4 2 0
  GETUPVAL R4 7
  CALL R4 0 1
  JUMPIFNOT R4 [+9]
  GETUPVAL R7 5
  GETTABLEKS R6 R7 K22 ["Terrain"]
  FASTCALL2 TABLE_INSERT R3 R6 [+4]
  MOVE R5 R3
  GETIMPORT R4 K13 [table.insert]
  CALL R4 2 0
  GETIMPORT R4 K24 [Enum.RaycastFilterType.Blacklist]
  SETTABLEKS R4 R2 K18 ["FilterType"]
  SETTABLEKS R3 R2 K19 ["FilterDescendantsInstances"]
  GETUPVAL R5 1
  GETUPVAL R7 2
  GETTABLEKS R6 R7 K7 ["BrushSettings"]
  GETTABLE R4 R5 R6
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K25 ["IgnoreWater"]
  GETTABLE R3 R4 R5
  SETTABLEKS R3 R2 K25 ["IgnoreWater"]
  LOADB R3 1
  SETTABLEKS R3 R2 K26 ["BruteForceAllSlow"]
  GETUPVAL R3 5
  MOVE R5 R0
  GETUPVAL R8 8
  GETTABLEKS R7 R8 K27 ["RayLength"]
  MUL R6 R1 R7
  MOVE R7 R2
  NAMECALL R3 R3 K28 ["Raycast"]
  CALL R3 4 1
  GETUPVAL R4 9
  NAMECALL R4 R4 K29 ["IsMouseDown"]
  CALL R4 1 1
  JUMPIFNOT R4 [+98]
  GETUPVAL R4 10
  CALL R4 0 1
  JUMPIFNOT R4 [+95]
  JUMPIFNOT R3 [+3]
  GETTABLEKS R4 R3 K1 ["Position"]
  JUMP [+3]
  GETUPVAL R5 11
  GETTABLEKS R4 R5 K1 ["Position"]
  GETUPVAL R7 12
  GETTABLEKS R6 R7 K30 ["CachePoint"]
  SUB R5 R4 R6
  GETTABLEKS R6 R5 K31 ["magnitude"]
  GETUPVAL R8 12
  GETTABLEKS R7 R8 K32 ["CacheID"]
  GETUPVAL R9 13
  CALL R9 0 1
  GETUPVAL R11 8
  GETTABLEKS R10 R11 K33 ["TerrainCacheUpdateThreshold"]
  MUL R8 R9 R10
  JUMPIFLT R8 R6 [+2]
  JUMPIF R7 [+18]
  GETUPVAL R8 5
  GETUPVAL R12 12
  GETTABLEKS R11 R12 K32 ["CacheID"]
  ORK R10 R11 K34 [""]
  MOVE R11 R4
  GETUPVAL R12 13
  CALL R12 0 -1
  NAMECALL R8 R8 K35 ["CacheCurrentTerrain"]
  CALL R8 -1 1
  MOVE R7 R8
  GETUPVAL R8 14
  NEWCLOSURE R9 P0
  CAPTURE UPVAL U15
  CAPTURE REF R7
  CAPTURE VAL R4
  CALL R8 1 0
  GETUPVAL R8 7
  CALL R8 0 1
  JUMPIFNOT R8 [+35]
  GETUPVAL R10 1
  GETUPVAL R12 2
  GETTABLEKS R11 R12 K7 ["BrushSettings"]
  GETTABLE R9 R10 R11
  GETUPVAL R11 3
  GETTABLEKS R10 R11 K8 ["IgnoreParts"]
  GETTABLE R8 R9 R10
  JUMPIF R8 [+25]
  GETUPVAL R8 5
  MOVE R10 R7
  MOVE R11 R0
  GETUPVAL R14 8
  GETTABLEKS R13 R14 K27 ["RayLength"]
  MUL R12 R1 R13
  LOADB R13 1
  MOVE R14 R2
  NAMECALL R8 R8 K36 ["RaycastCachedTerrain"]
  CALL R8 6 1
  JUMPIFNOT R3 [+9]
  JUMPIFNOT R8 [+8]
  GETTABLEKS R9 R8 K37 ["Distance"]
  GETTABLEKS R10 R3 K37 ["Distance"]
  JUMPIFNOTLT R9 R10 [+19]
  MOVE R3 R8
  JUMP [+16]
  JUMPIFNOT R8 [+15]
  MOVE R3 R8
  JUMP [+13]
  GETUPVAL R8 5
  MOVE R10 R7
  MOVE R11 R0
  GETUPVAL R14 8
  GETTABLEKS R13 R14 K27 ["RayLength"]
  MUL R12 R1 R13
  LOADB R13 1
  MOVE R14 R2
  NAMECALL R8 R8 K36 ["RaycastCachedTerrain"]
  CALL R8 6 1
  MOVE R3 R8
  CLOSEUPVALS R7
  JUMPIFNOT R3 [+40]
  DUPTABLE R4 K41 [{"Distance", "Instance", "Material", "Normal", "Position"}]
  GETTABLEKS R7 R3 K1 ["Position"]
  SUB R6 R7 R0
  GETTABLEKS R5 R6 K42 ["Magnitude"]
  SETTABLEKS R5 R4 K37 ["Distance"]
  GETTABLEKS R5 R3 K38 ["Instance"]
  SETTABLEKS R5 R4 K38 ["Instance"]
  GETTABLEKS R5 R3 K39 ["Material"]
  SETTABLEKS R5 R4 K39 ["Material"]
  GETTABLEKS R5 R3 K40 ["Normal"]
  SETTABLEKS R5 R4 K40 ["Normal"]
  GETTABLEKS R5 R3 K1 ["Position"]
  SETTABLEKS R5 R4 K1 ["Position"]
  MOVE R3 R4
  GETUPVAL R5 16
  GETTABLEKS R4 R5 K43 ["current"]
  GETUPVAL R6 17
  GETTABLEKS R5 R6 K44 ["Pending"]
  JUMPIFNOTEQ R4 R5 [+90]
  GETUPVAL R4 16
  GETUPVAL R6 17
  GETTABLEKS R5 R6 K45 ["Active"]
  SETTABLEKS R5 R4 K43 ["current"]
  JUMP [+82]
  GETUPVAL R5 16
  GETTABLEKS R4 R5 K43 ["current"]
  JUMPIFNOTEQKS R4 K45 ["Active"] [+21]
  DUPTABLE R4 K41 [{"Distance", "Instance", "Material", "Normal", "Position"}]
  LOADN R5 255
  SETTABLEKS R5 R4 K37 ["Distance"]
  LOADNIL R5
  SETTABLEKS R5 R4 K38 ["Instance"]
  GETIMPORT R5 K47 [Enum.Material.Air]
  SETTABLEKS R5 R4 K39 ["Material"]
  GETIMPORT R5 K50 [Vector3.yAxis]
  SETTABLEKS R5 R4 K40 ["Normal"]
  GETIMPORT R5 K52 [Vector3.zero]
  SETTABLEKS R5 R4 K1 ["Position"]
  RETURN R4 1
  GETUPVAL R4 18
  CALL R4 0 1
  GETUPVAL R5 19
  MOVE R6 R0
  MOVE R7 R1
  GETIMPORT R8 K52 [Vector3.zero]
  GETIMPORT R9 K50 [Vector3.yAxis]
  CALL R5 4 2
  JUMPIFNOT R5 [+16]
  JUMPIFNOT R6 [+15]
  LOADN R7 0
  JUMPIFNOTLE R7 R6 [+13]
  DUPTABLE R7 K53 [{"Distance", "Normal", "Position"}]
  SUB R9 R5 R0
  GETTABLEKS R8 R9 K42 ["Magnitude"]
  SETTABLEKS R8 R7 K37 ["Distance"]
  SETTABLEKS R4 R7 K40 ["Normal"]
  SETTABLEKS R5 R7 K1 ["Position"]
  MOVE R3 R7
  JUMP [+16]
  GETUPVAL R10 8
  GETTABLEKS R9 R10 K27 ["RayLength"]
  MUL R8 R1 R9
  ADD R7 R0 R8
  DUPTABLE R8 K53 [{"Distance", "Normal", "Position"}]
  SUB R10 R7 R0
  GETTABLEKS R9 R10 K42 ["Magnitude"]
  SETTABLEKS R9 R8 K37 ["Distance"]
  SETTABLEKS R4 R8 K40 ["Normal"]
  SETTABLEKS R7 R8 K1 ["Position"]
  MOVE R3 R8
  GETUPVAL R8 16
  GETTABLEKS R7 R8 K43 ["current"]
  GETUPVAL R9 17
  GETTABLEKS R8 R9 K44 ["Pending"]
  JUMPIFNOTEQ R7 R8 [+7]
  GETUPVAL R7 16
  GETUPVAL R9 17
  GETTABLEKS R8 R9 K54 ["Inactive"]
  SETTABLEKS R8 R7 K43 ["current"]
  GETUPVAL R4 20
  GETUPVAL R5 1
  GETUPVAL R7 9
  NAMECALL R7 R7 K29 ["IsMouseDown"]
  CALL R7 1 1
  JUMPIFNOT R7 [+2]
  GETUPVAL R6 21
  JUMP [+10]
  GETIMPORT R6 K57 [CFrame.lookAt]
  GETTABLEKS R7 R3 K1 ["Position"]
  GETTABLEKS R9 R3 K1 ["Position"]
  GETUPVAL R10 18
  CALL R10 0 1
  ADD R8 R9 R10
  CALL R6 2 1
  GETUPVAL R7 0
  GETUPVAL R8 22
  CALL R4 4 1
  JUMPIFNOT R4 [+1]
  MOVE R3 R4
  GETUPVAL R7 1
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K7 ["BrushSettings"]
  GETTABLE R6 R7 R8
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K58 ["Snapping"]
  GETTABLE R5 R6 R7
  GETUPVAL R6 23
  GETIMPORT R7 K60 [table.clone]
  MOVE R8 R3
  CALL R7 1 -1
  CALL R6 -1 0
  JUMPIFEQKNIL R5 [+45]
  GETUPVAL R7 24
  GETTABLEKS R6 R7 K61 ["Off"]
  JUMPIFEQ R5 R6 [+40]
  GETUPVAL R6 25
  GETTABLEKS R7 R3 K1 ["Position"]
  CALL R6 1 1
  GETUPVAL R9 1
  GETUPVAL R11 2
  GETTABLEKS R10 R11 K7 ["BrushSettings"]
  GETTABLE R8 R9 R10
  GETUPVAL R10 3
  GETTABLEKS R9 R10 K62 ["FixedYPlane"]
  GETTABLE R7 R8 R9
  JUMPIFNOT R7 [+18]
  GETTABLEKS R8 R6 K63 ["X"]
  GETUPVAL R11 1
  GETUPVAL R13 2
  GETTABLEKS R12 R13 K7 ["BrushSettings"]
  GETTABLE R10 R11 R12
  GETUPVAL R12 3
  GETTABLEKS R11 R12 K64 ["FixedYPlanePosition"]
  GETTABLE R9 R10 R11
  GETTABLEKS R10 R6 K65 ["Z"]
  FASTCALL VECTOR [+2]
  GETIMPORT R7 K66 [Vector3.new]
  CALL R7 3 1
  MOVE R6 R7
  SUB R8 R6 R0
  GETTABLEKS R7 R8 K42 ["Magnitude"]
  SETTABLEKS R7 R3 K37 ["Distance"]
  SETTABLEKS R6 R3 K1 ["Position"]
  GETUPVAL R6 26
  GETTABLEKS R7 R3 K1 ["Position"]
  SETTABLEKS R7 R6 K43 ["current"]
  RETURN R3 1

PROTO_7:
  GETTABLEKS R1 R0 K0 ["CacheID"]
  JUMPIFNOT R1 [+1]
  RETURN R0 1
  DUPTABLE R1 K2 [{"CacheID", "CachePoint"}]
  GETUPVAL R2 0
  LOADK R4 K3 [""]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K4 ["Position"]
  GETUPVAL R6 2
  CALL R6 0 -1
  NAMECALL R2 R2 K5 ["CacheCurrentTerrain"]
  CALL R2 -1 1
  SETTABLEKS R2 R1 K0 ["CacheID"]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K4 ["Position"]
  SETTABLEKS R2 R1 K1 ["CachePoint"]
  RETURN R1 1

PROTO_8:
  GETUPVAL R0 0
  CALL R0 0 1
  JUMPIFNOT R0 [+12]
  GETUPVAL R0 1
  NEWCLOSURE R1 P0
  CAPTURE UPVAL U2
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CALL R0 1 0
  GETUPVAL R0 5
  GETUPVAL R2 6
  GETTABLEKS R1 R2 K0 ["Pending"]
  SETTABLEKS R1 R0 K1 ["current"]
  RETURN R0 0

PROTO_9:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["CacheID"]
  JUMPIFNOT R0 [+7]
  GETUPVAL R0 1
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["CacheID"]
  NAMECALL R0 R0 K1 ["ClearCachedTerrain"]
  CALL R0 2 0
  GETUPVAL R0 2
  DUPTABLE R1 K3 [{"CachePoint"}]
  GETIMPORT R2 K6 [Vector3.zero]
  SETTABLEKS R2 R1 K2 ["CachePoint"]
  CALL R0 1 0
  GETUPVAL R0 3
  LOADNIL R1
  SETTABLEKS R1 R0 K7 ["current"]
  RETURN R0 0

PROTO_10:
  GETUPVAL R0 0
  JUMPIFNOT R0 [+3]
  GETUPVAL R0 1
  LOADB R1 0
  CALL R0 1 0
  RETURN R0 0

PROTO_11:
  GETUPVAL R0 0
  NAMECALL R0 R0 K0 ["isMouseMoveReserved"]
  CALL R0 1 1
  JUMPIF R0 [+9]
  GETUPVAL R0 1
  GETUPVAL R1 2
  GETUPVAL R2 0
  NAMECALL R2 R2 K1 ["IsMouseDown"]
  CALL R2 1 -1
  CALL R1 -1 -1
  CALL R0 -1 0
  RETURN R0 0
  GETUPVAL R0 3
  JUMPIF R0 [+3]
  GETUPVAL R0 4
  LOADB R1 1
  CALL R0 1 0
  RETURN R0 0

PROTO_12:
  GETUPVAL R0 0
  LOADK R2 K0 ["TerrainRaycast"]
  NAMECALL R0 R0 K1 ["UnbindFromRenderStep"]
  CALL R0 2 0
  RETURN R0 0

PROTO_13:
  GETUPVAL R0 0
  LOADK R2 K0 ["TerrainRaycast"]
  LOADN R3 1
  NEWCLOSURE R4 P0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CAPTURE UPVAL U5
  NAMECALL R0 R0 K1 ["BindToRenderStep"]
  CALL R0 4 0
  DUPCLOSURE R0 K2 [PROTO_12]
  CAPTURE UPVAL U0
  RETURN R0 1

PROTO_14:
  GETUPVAL R3 0
  NAMECALL R3 R3 K0 ["use"]
  CALL R3 1 1
  NAMECALL R4 R3 K1 ["getMouse"]
  CALL R4 1 1
  GETUPVAL R5 1
  DUPTABLE R6 K5 [{"Distance", "Normal", "Position"}]
  GETTABLEKS R9 R4 K6 ["Origin"]
  GETTABLEKS R8 R9 K4 ["Position"]
  GETTABLEKS R7 R8 K7 ["Magnitude"]
  SETTABLEKS R7 R6 K2 ["Distance"]
  GETIMPORT R7 K10 [Vector3.yAxis]
  SETTABLEKS R7 R6 K3 ["Normal"]
  GETIMPORT R7 K12 [Vector3.zero]
  SETTABLEKS R7 R6 K4 ["Position"]
  CALL R5 1 2
  GETUPVAL R7 1
  DUPTABLE R8 K5 [{"Distance", "Normal", "Position"}]
  GETTABLEKS R11 R4 K6 ["Origin"]
  GETTABLEKS R10 R11 K4 ["Position"]
  GETTABLEKS R9 R10 K7 ["Magnitude"]
  SETTABLEKS R9 R8 K2 ["Distance"]
  GETIMPORT R9 K10 [Vector3.yAxis]
  SETTABLEKS R9 R8 K3 ["Normal"]
  GETIMPORT R9 K12 [Vector3.zero]
  SETTABLEKS R9 R8 K4 ["Position"]
  CALL R7 1 2
  GETUPVAL R9 1
  GETIMPORT R10 K15 [CFrame.lookAt]
  GETTABLEKS R11 R5 K4 ["Position"]
  GETTABLEKS R13 R5 K4 ["Position"]
  GETIMPORT R14 K10 [Vector3.yAxis]
  ADD R12 R13 R14
  CALL R10 2 -1
  CALL R9 -1 2
  GETUPVAL R11 2
  NEWCLOSURE R12 P0
  CAPTURE VAL R3
  CAPTURE VAL R5
  CAPTURE UPVAL U3
  CAPTURE VAL R10
  NEWTABLE R13 0 2
  MOVE R14 R0
  MOVE R15 R5
  SETLIST R13 R14 2 [1]
  CALL R11 2 0
  GETUPVAL R11 1
  DUPTABLE R12 K18 [{"CacheID", "CachePoint"}]
  LOADNIL R13
  SETTABLEKS R13 R12 K16 ["CacheID"]
  GETIMPORT R13 K12 [Vector3.zero]
  SETTABLEKS R13 R12 K17 ["CachePoint"]
  CALL R11 1 2
  GETUPVAL R13 1
  NAMECALL R14 R3 K19 ["isMouseMoveReserved"]
  CALL R14 1 -1
  CALL R13 -1 2
  GETUPVAL R15 4
  LOADNIL R16
  CALL R15 1 1
  GETUPVAL R16 4
  GETIMPORT R17 K12 [Vector3.zero]
  CALL R16 1 1
  GETUPVAL R17 5
  CALL R17 0 1
  GETUPVAL R20 6
  GETTABLEKS R19 R20 K20 ["MaterialSettings"]
  GETTABLE R18 R1 R19
  GETUPVAL R19 7
  NEWCLOSURE R20 P1
  CAPTURE VAL R1
  CAPTURE UPVAL U6
  CAPTURE UPVAL U8
  CAPTURE UPVAL U9
  NEWTABLE R21 0 1
  GETUPVAL R25 6
  GETTABLEKS R24 R25 K21 ["BrushSettings"]
  GETTABLE R23 R1 R24
  GETUPVAL R25 8
  GETTABLEKS R24 R25 K22 ["BrushSize"]
  GETTABLE R22 R23 R24
  SETLIST R21 R22 1 [1]
  CALL R19 2 1
  GETUPVAL R20 7
  NEWCLOSURE R21 P2
  CAPTURE VAL R0
  CAPTURE UPVAL U10
  CAPTURE VAL R1
  CAPTURE UPVAL U6
  CAPTURE UPVAL U8
  CAPTURE VAL R18
  CAPTURE UPVAL U11
  CAPTURE UPVAL U12
  NEWTABLE R22 0 4
  GETUPVAL R26 6
  GETTABLEKS R25 R26 K21 ["BrushSettings"]
  GETTABLE R24 R1 R25
  GETUPVAL R26 8
  GETTABLEKS R25 R26 K23 ["TemporarySmooth"]
  GETTABLE R23 R24 R25
  GETUPVAL R27 6
  GETTABLEKS R26 R27 K21 ["BrushSettings"]
  GETTABLE R25 R1 R26
  GETUPVAL R27 8
  GETTABLEKS R26 R27 K24 ["PlaneLock"]
  GETTABLE R24 R25 R26
  JUMPIFNOT R18 [+5]
  GETUPVAL R27 11
  GETTABLEKS R26 R27 K25 ["TargetMaterial"]
  GETTABLE R25 R18 R26
  JUMP [+1]
  LOADB R25 1
  MOVE R26 R0
  SETLIST R22 R23 4 [1]
  CALL R20 2 1
  GETUPVAL R21 7
  NEWCLOSURE R22 P3
  CAPTURE VAL R1
  CAPTURE UPVAL U6
  CAPTURE UPVAL U8
  CAPTURE UPVAL U9
  CAPTURE UPVAL U13
  CAPTURE UPVAL U14
  CAPTURE VAL R16
  CAPTURE UPVAL U15
  NEWTABLE R23 0 1
  MOVE R24 R1
  SETLIST R23 R24 1 [1]
  CALL R21 2 1
  GETUPVAL R22 7
  NEWCLOSURE R23 P4
  CAPTURE UPVAL U6
  CAPTURE UPVAL U8
  CAPTURE VAL R0
  CAPTURE UPVAL U10
  CAPTURE VAL R21
  CAPTURE VAL R6
  CAPTURE VAL R2
  NEWTABLE R24 0 4
  MOVE R25 R9
  MOVE R26 R2
  MOVE R27 R0
  MOVE R28 R21
  SETLIST R24 R25 4 [1]
  CALL R22 2 1
  GETUPVAL R23 7
  NEWCLOSURE R24 P5
  CAPTURE VAL R4
  CAPTURE VAL R1
  CAPTURE UPVAL U6
  CAPTURE UPVAL U8
  CAPTURE VAL R17
  CAPTURE UPVAL U16
  CAPTURE UPVAL U17
  CAPTURE UPVAL U18
  CAPTURE UPVAL U9
  CAPTURE VAL R3
  CAPTURE VAL R20
  CAPTURE VAL R5
  CAPTURE VAL R11
  CAPTURE VAL R19
  CAPTURE VAL R12
  CAPTURE UPVAL U19
  CAPTURE VAL R15
  CAPTURE UPVAL U20
  CAPTURE UPVAL U3
  CAPTURE UPVAL U21
  CAPTURE UPVAL U22
  CAPTURE VAL R9
  CAPTURE VAL R0
  CAPTURE VAL R8
  CAPTURE UPVAL U23
  CAPTURE UPVAL U24
  CAPTURE VAL R16
  NEWTABLE R25 0 12
  NAMECALL R27 R3 K26 ["IsMouseDown"]
  CALL R27 1 1
  JUMPIFNOT R27 [+2]
  MOVE R26 R9
  JUMP [+1]
  LOADB R26 1
  MOVE R27 R11
  MOVE R28 R19
  MOVE R29 R20
  GETUPVAL R33 6
  GETTABLEKS R32 R33 K21 ["BrushSettings"]
  GETTABLE R31 R1 R32
  GETUPVAL R33 8
  GETTABLEKS R32 R33 K27 ["IgnoreParts"]
  GETTABLE R30 R31 R32
  GETUPVAL R34 6
  GETTABLEKS R33 R34 K21 ["BrushSettings"]
  GETTABLE R32 R1 R33
  GETUPVAL R34 8
  GETTABLEKS R33 R34 K28 ["IgnoreWater"]
  GETTABLE R31 R32 R33
  GETUPVAL R35 6
  GETTABLEKS R34 R35 K21 ["BrushSettings"]
  GETTABLE R33 R1 R34
  GETUPVAL R35 8
  GETTABLEKS R34 R35 K29 ["Snapping"]
  GETTABLE R32 R33 R34
  GETUPVAL R36 6
  GETTABLEKS R35 R36 K21 ["BrushSettings"]
  GETTABLE R34 R1 R35
  GETUPVAL R36 8
  GETTABLEKS R35 R36 K30 ["FixedYPlane"]
  GETTABLE R33 R34 R35
  GETUPVAL R37 6
  GETTABLEKS R36 R37 K21 ["BrushSettings"]
  GETTABLE R35 R1 R36
  GETUPVAL R37 8
  GETTABLEKS R36 R37 K31 ["FixedYPlanePosition"]
  GETTABLE R34 R35 R36
  GETUPVAL R38 6
  GETTABLEKS R37 R38 K21 ["BrushSettings"]
  GETTABLE R36 R1 R37
  GETUPVAL R38 8
  GETTABLEKS R37 R38 K32 ["PlaneTransform"]
  GETTABLE R35 R36 R37
  GETUPVAL R39 6
  GETTABLEKS R38 R39 K21 ["BrushSettings"]
  GETTABLE R37 R1 R38
  GETUPVAL R39 8
  GETTABLEKS R38 R39 K24 ["PlaneLock"]
  GETTABLE R36 R37 R38
  MOVE R37 R0
  SETLIST R25 R26 12 [1]
  CALL R23 2 1
  GETUPVAL R24 25
  GETTABLEKS R25 R4 K33 ["Button1Down"]
  NEWCLOSURE R26 P6
  CAPTURE VAL R20
  CAPTURE VAL R12
  CAPTURE UPVAL U16
  CAPTURE VAL R5
  CAPTURE VAL R19
  CAPTURE VAL R15
  CAPTURE UPVAL U20
  NEWTABLE R27 0 1
  MOVE R28 R5
  SETLIST R27 R28 1 [1]
  CALL R24 3 0
  GETUPVAL R24 25
  GETTABLEKS R25 R4 K34 ["Button1Up"]
  NEWCLOSURE R26 P7
  CAPTURE VAL R11
  CAPTURE UPVAL U16
  CAPTURE VAL R12
  CAPTURE VAL R15
  NEWTABLE R27 0 1
  MOVE R28 R11
  SETLIST R27 R28 1 [1]
  CALL R24 3 0
  GETUPVAL R24 25
  GETTABLEKS R25 R3 K35 ["MouseMoveAction"]
  NEWCLOSURE R26 P8
  CAPTURE VAL R13
  CAPTURE VAL R14
  NEWTABLE R27 0 1
  MOVE R28 R13
  SETLIST R27 R28 1 [1]
  CALL R24 3 0
  GETUPVAL R24 2
  NEWCLOSURE R25 P9
  CAPTURE UPVAL U26
  CAPTURE VAL R3
  CAPTURE VAL R22
  CAPTURE VAL R23
  CAPTURE VAL R13
  CAPTURE VAL R14
  NEWTABLE R26 0 1
  MOVE R27 R22
  SETLIST R26 R27 1 [1]
  CALL R24 2 0
  MOVE R24 R5
  MOVE R25 R7
  GETTABLEKS R26 R16 K36 ["current"]
  RETURN R24 3

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["TerrainEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Dash"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETTABLEKS R3 R2 K9 ["useCallback"]
  GETTABLEKS R4 R2 K10 ["useEffect"]
  GETTABLEKS R5 R2 K11 ["useRef"]
  GETTABLEKS R6 R2 K12 ["useState"]
  GETIMPORT R7 K5 [require]
  GETTABLEKS R10 R0 K13 ["Src"]
  GETTABLEKS R9 R10 K14 ["Controllers"]
  GETTABLEKS R8 R9 K15 ["ShortcutController"]
  CALL R7 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R11 R0 K13 ["Src"]
  GETTABLEKS R10 R11 K16 ["Hooks"]
  GETTABLEKS R9 R10 K17 ["useEventConnection"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K13 ["Src"]
  GETTABLEKS R11 R12 K16 ["Hooks"]
  GETTABLEKS R10 R11 K18 ["useTerrain"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R13 R0 K13 ["Src"]
  GETTABLEKS R12 R13 K19 ["Resources"]
  GETTABLEKS R11 R12 K20 ["Constants"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K13 ["Src"]
  GETTABLEKS R13 R14 K21 ["Util"]
  GETTABLEKS R12 R13 K22 ["ApplyPlaneLock"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R15 R0 K13 ["Src"]
  GETTABLEKS R14 R15 K21 ["Util"]
  GETTABLEKS R13 R14 K23 ["ConvertForPivot"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETTABLEKS R16 R0 K13 ["Src"]
  GETTABLEKS R15 R16 K21 ["Util"]
  GETTABLEKS R14 R15 K24 ["getCameraLookPlane"]
  CALL R13 1 1
  GETIMPORT R14 K5 [require]
  GETTABLEKS R17 R0 K13 ["Src"]
  GETTABLEKS R16 R17 K21 ["Util"]
  GETTABLEKS R15 R16 K25 ["getPlaneIntersection"]
  CALL R14 1 1
  GETIMPORT R15 K5 [require]
  GETTABLEKS R18 R0 K13 ["Src"]
  GETTABLEKS R17 R18 K21 ["Util"]
  GETTABLEKS R16 R17 K26 ["getSnappedToVoxel"]
  CALL R15 1 1
  GETIMPORT R16 K28 [game]
  LOADK R18 K29 ["CoreGui"]
  NAMECALL R16 R16 K30 ["GetService"]
  CALL R16 2 1
  GETIMPORT R17 K28 [game]
  LOADK R19 K31 ["Players"]
  NAMECALL R17 R17 K30 ["GetService"]
  CALL R17 2 1
  GETIMPORT R18 K28 [game]
  LOADK R20 K32 ["RunService"]
  NAMECALL R18 R18 K30 ["GetService"]
  CALL R18 2 1
  GETIMPORT R19 K28 [game]
  LOADK R21 K33 ["Workspace"]
  NAMECALL R19 R19 K30 ["GetService"]
  CALL R19 2 1
  GETIMPORT R20 K5 [require]
  GETTABLEKS R22 R0 K13 ["Src"]
  GETTABLEKS R21 R22 K34 ["Types"]
  CALL R20 1 1
  GETTABLEKS R21 R20 K35 ["BrushSettings"]
  GETTABLEKS R22 R20 K36 ["BrushShape"]
  GETTABLEKS R23 R20 K37 ["Category"]
  GETTABLEKS R24 R20 K38 ["MaterialSettings"]
  GETTABLEKS R25 R20 K39 ["PlaneLock"]
  GETTABLEKS R26 R20 K40 ["RaycastType"]
  GETTABLEKS R27 R20 K41 ["Snapping"]
  GETTABLEKS R28 R20 K42 ["Tool"]
  GETIMPORT R29 K5 [require]
  GETTABLEKS R32 R0 K13 ["Src"]
  GETTABLEKS R31 R32 K43 ["Flags"]
  GETTABLEKS R30 R31 K44 ["getFFlagTerrainEditorPartsInFrontOfCacheFix"]
  CALL R29 1 1
  DUPCLOSURE R30 K45 [PROTO_14]
  CAPTURE VAL R7
  CAPTURE VAL R6
  CAPTURE VAL R4
  CAPTURE VAL R13
  CAPTURE VAL R5
  CAPTURE VAL R9
  CAPTURE VAL R23
  CAPTURE VAL R3
  CAPTURE VAL R21
  CAPTURE VAL R10
  CAPTURE VAL R28
  CAPTURE VAL R24
  CAPTURE VAL R25
  CAPTURE VAL R22
  CAPTURE VAL R12
  CAPTURE VAL R16
  CAPTURE VAL R19
  CAPTURE VAL R17
  CAPTURE VAL R29
  CAPTURE VAL R1
  CAPTURE VAL R26
  CAPTURE VAL R14
  CAPTURE VAL R11
  CAPTURE VAL R27
  CAPTURE VAL R15
  CAPTURE VAL R8
  CAPTURE VAL R18
  RETURN R30 1
